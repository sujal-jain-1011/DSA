...
/*Function to search an element (given by user as input) in binary search tree using recursion*/
short unsigned int isFound(struct TreeNode** root_add, int element)
{
    ...
    if(*(root_add))
    {
        ...
        if yes then we are going to return one otherwise we are going to call the isFound func-
        tion recusively for left or right subtree according to the value of the element*/
        if((*(root_add))->val==element) return 1;
