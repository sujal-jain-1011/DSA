...
/*Function to search an element (given by user as input) in binary search tree using iterative approach*/
short unsigned int isFound(struct TreeNode** root_add, int element)
{
    ...
    while(ptr)
    {
        ...
        we are going to return one from here only indicating that element is found*/
        else return 1;
    }
...
int main(void)
{
    ...
    if(createBinarySearchTree())
    {
        ...
        printf("Enter element to be found in binary search tree:");
        scanf("%d", &elm); 
        if(isFound(&root, elm)) printf("%d is found in binary search tree\n", elm);
        else printf("%d is not found in binary search tree\n", elm);
