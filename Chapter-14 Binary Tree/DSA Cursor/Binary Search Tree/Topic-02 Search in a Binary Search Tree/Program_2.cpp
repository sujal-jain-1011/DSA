...
and search an element in the binary search tree from the given user input using iterative approach*/
...
class BinarySearchTree{
    ...
    public:
        ...
        /*Member function of class: to search an element (given by user as input) in binary search tree using iterative approach*/
        bool isFound(struct TreeNode** root_add, int element)
        {
            /*Running a loop to find the element in the binary search tree, if current element
            is greater than the element to be found then we are going to search the element
            in the left subtree of the current node else in the right subtree (if not equal)*/
            struct TreeNode* ptr=(*root_add);
            while(ptr)
            {
                /*If current node element is greater than the element to be found, then we
                are going to search the element in the left subtree of the current node*/
                if(ptr->val>element) ptr=ptr->left;

                /*Else if current node element is lesser than the element to be found, then
                we are going to search the element in the right subtree of the current node*/
                else if(ptr->val<element) ptr=ptr->right;

                /*Else the current node element is equal to the element to be found, then
                we are going to return true from here only indicating that element is found*/
                else return true;
            }

            /*Returning zero since we are out of the loop and the desired element is not found*/
            return false; 
        }
...
int main(void)
{
    ...
    if(BST.createBinarySearchTree())
    {
        ...
        cout<<"Enter element to be found in binary search tree:";
        cin>>elm;
        if(BST.isFound(&(BST.root), elm)) cout<<elm<<" is found in binary search tree"<<endl;
        else cout<<elm<<" is not found in binary search tree"<<endl;
