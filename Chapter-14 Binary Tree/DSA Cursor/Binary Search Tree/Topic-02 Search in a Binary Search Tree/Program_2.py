...
class BinarySearchTree:
        ...
        def isFound(self, root_add, element):
            ...
            while(ptr):
                ...
                are going to search the element in the left subtree of the current node else if
                current node element is lesser than the element to be found, then we are going to
                search the element in the right subtree of the current node else the current node
                element is equal to the element to be found, then we are going to return true from
                here only indicating that element is found in the binary search tree"""
                if(ptr.val>element):
                    ptr=ptr.left
                elif(ptr.val<element):
                    ptr=ptr.right
                else:
                    return True
...
if(BST.createBinarySearchTree()):
    ...
    elm=int(input("Enter element to be found in binary search tree:"))
    if(BST.isFound(BST.root, elm)):
        print(f"{elm} is found in binary search tree")
    else:
        print(f"{elm} is not found in binary search tree")
else:
