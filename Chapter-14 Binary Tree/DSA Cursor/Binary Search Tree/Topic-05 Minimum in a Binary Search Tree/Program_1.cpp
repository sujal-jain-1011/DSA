...
array and to find the minimum element in the binary search tree using recursive approach*/
...
class BinarySearchTree{
    ...
    public:
        ...
        /*Member function of class: to find the minimum element in binary search tree using recursive approach*/
        int minBST(struct TreeNode** root_add)
        {
            /*Declaring base case for recursion: when left subtree of binary search tree is/becomes empty*/
            if((*(root_add))->left) return minBST(&((*(root_add))->left)); else return (*(root_add))->val;
        }
...
int main(void)
{
    ...
    if(BST.createBinarySearchTree())
    {
        ...
        if(!(BST.root)) cout<<"Can't find minimum since binary search tree is empty!"<<endl;
        else cout<<"The minimum element in binary search tree is:"<<BST.minBST(&(BST.root))<<endl;
