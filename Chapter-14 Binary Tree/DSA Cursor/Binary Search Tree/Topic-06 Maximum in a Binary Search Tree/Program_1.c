...
/*Function to find the maximum element in binary search tree using recursive approach*/
...
int main(void)
{
    ...
    if(createBinarySearchTree())
    {
        ...
        if(!root) printf("Can't find maximum since binary search tree is empty!\n");
        else printf("The maximum element in binary search tree is:%d\n", maxBST(&root));
