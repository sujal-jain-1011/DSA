...
/*Function to populate an array with the inorder traversal of the binary tree*/
...
short unsigned int checkBST(struct TreeNode** root_add)
{
    ...
    int* inArr=(int*)malloc(capacity*sizeof(int)); long long unsigned int it=0;
    inorder(root_add, inArr, &it);

    /*Checking whether the array sorted in increasing order or not by running a loop*/
    for(long long unsigned int i=0; i<capacity-1; i++) if(inArr[i]>inArr[i+1]) return 0;
    return 1;
}
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        if(checkBST(&(root))) printf("Given binary tree is a valid binary search tree\n");
