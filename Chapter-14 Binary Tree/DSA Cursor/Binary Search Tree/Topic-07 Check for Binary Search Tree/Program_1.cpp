...
class BinaryTree{
    ...
    public:
        ...
        void inorder(struct TreeNode** root_add, int* arr, long long unsigned int* iter)
        {
            /*Declaring base case for recursion: when the binary tree is/becomes empty*/
            if((*root_add))
            {
                ...
                arr[(*iter)++]=(*(root_add))->val;
                inorder(&((*root_add)->right), arr, iter);
            }
        }

        /*Member function of class: to check whether the given binary tree is a valid binary search tree or
        not, using the fact that inorder traversal of binary search tree is in strictly increasing order*/
        bool checkBST(struct TreeNode** root_add)
        {
            /*We are going to call a function that will populate an array from the inorder traversal of the
            binary tree, then we'll check whether the array is sorted in strictly increasing order or not*/
            int* inArr=new(nothrow) int[capacity]; long long unsigned int it=0;
            inorder(root_add, inArr, &it);

            /*Checking whether the array sorted in increasing order or not by running a loop*/
            for(long long unsigned int i=0; i<capacity-1; i++) if(inArr[i]>inArr[i+1]) return false;
            return true;
        }
...
int main(void)
{
    ...
    if(BT.createBinaryTree())
    {
        ...
        if(BT.checkBST(&(BT.root))) cout<<"Given binary tree is a valid binary search tree"<<endl;
