...
/*Function to detect any off chart element in the given binary tree for checking whether given binary tree is BST*/
void check(struct TreeNode** root_add, struct TreeNode** ptr, short unsigned int* isfd)
{
    ...
    if((*(root_add)) && (*(isfd)))
    {
        ...
        if((*(ptr))){if((*(ptr))->val>=(*(root_add))->val) (*(isfd))=0;}
        (*(ptr))=(*root_add);

        /*Checking idfd variable before calling check function for right subtree*/
        if((*isfd)) check(&((*(root_add))->right), ptr, isfd);
    }
}
...
short unsigned int checkBST(struct TreeNode** root_add)
{
    ...
    struct TreeNode* ptr=NULL; short unsigned int isFound=1;
