...
class BinarySearchTree{
    ...
    public:
        ...
        struct ListNode* createListNode(int data)
        {
            /*Dynamically allocating memory for the new node using malloc function and checking if the memory allocation was
            successful if it was, then populating the node with the data passed to function and returning it's address*/
            struct ListNode* newListNode=new(nothrow) struct ListNode;
            if(newListNode==nullptr)
            {
                cout<<"Memory allocation failed for node of linked list!"<<endl;
                return nullptr;
            }
        ...
        bool createLinkedList(void)
        {
            ...
            llui size; cout<<"Enter the size of linked list:"; cin>>size;
            ...
            if(size>ULLONG_MAX || size<0)
            {
                ...
                cout<<"Invalid user input for size of linked list!"<<endl;
                return false;
            }

            /*Handling corner case: when capacity is zero*/
            if(size==0) {cout<<"Linked List is empty!"<<endl; return false;}
            ...
            for(llui iter=0; iter<size; iter++)
            {
                ...
                int elm; cout<<"Enter the value of node "<<(iter+1)<<":";
                cin>>elm; struct ListNode* temp=createListNode(elm);
            ...
            int *arr=new(nothrow) int[size]; llui i=0;
            ...
            while(lptr){arr[i++]=lptr->val; lptr=lptr->next;} sort(arr, arr+size);
            ...
            root=createBST(lptr, nullptr); return true;
        }
