...
and we are going to find the level order traversal of the binary tree using queue*/
    ...
    else
    {
        ...
        else
        {
            ...
            while(!isEmpty(frontLOT, rearLOT))
            {
                ...
                if(qptr)
                {
                    /*Printing the value of the current node and enqueuing child nodes if they exist*/
                    printf("%d ", qptr->val);
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        printf("The level order traversal of binary tree is:\n");
