...
class BinaryTree{
    ...
    public:
        ...
        /*Member function of class: to find the maximum of two integers*/
        int max(int a, int b) {return a>=b?a:b;}

        /*Member function of class: to find the maximum element in binary tree using recursive approach*/
        int maxInBinaryTree(struct TreeNode** root_add)
        {
            /*Declaring base case for recursion: when binary tree is/becomes empty, in that case we are going to ret-
            urn INT_MIN else we are going to return maximum of values from left and right subtrees and current node*/
            if(!(*root_add)) return INT_MIN;
            else return max(max(maxInBinaryTree(&((*root_add)->left)), maxInBinaryTree(&((*root_add)->right))), (*root_add)->val);
        }
};
...
int main(void)
{
    ...
    if(BT.createBinaryTree())
    {
        ...
        cout<<"The maximum element in binary tree is:"<<BT.maxInBinaryTree(&(BT.root))<<endl;
    }
