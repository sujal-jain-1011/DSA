...
class BinaryTree{
    ...
    int maxInBinaryTree(TreeNode root_add)
    {
        /*Declaring base case for recursion: when binary tree is/becomes empty, in that case we are going to ret-
        urn Integer.MIN_VALUE else we are going to return maximum of values from left and right subtrees and current node*/
        if(root_add==null) return Integer.MIN_VALUE;
        else return Math.max(Math.max(maxInBinaryTree(root_add.left), maxInBinaryTree(root_add.right)), root_add.val);
    }
};
...
public class Program_1
{
    ...
    public static void main(String[] args)
    {
        ...
        if(BT.createBinaryTree())
        {
            ...
            System.out.println("The maximum element in binary tree is:"+BT.maxInBinaryTree(BT.root));
        }
        else
