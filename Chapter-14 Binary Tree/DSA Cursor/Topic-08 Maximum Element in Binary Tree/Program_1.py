...
class BinaryTree:
        ...
        def maxInBinaryTree(self, root_add):
            ...
            return -inf else we are going to return maximum of values from left and right subtrees and current node"""
            if(not(root_add)):
                return -inf
            else:
                return max(max(self.maxInBinaryTree(root_add.left), self.maxInBinaryTree(root_add.right)), root_add.val)
