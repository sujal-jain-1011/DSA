...
class BinaryTree{
    ...
    int minInBinaryTree(TreeNode root_add)
    {
        ...
        urn Integer.MIN_VALUE else we are going to return minimum of values from left and right subtrees and current node*/
        if(root_add==null) return Integer.MAX_VALUE;
        else return Math.min(Math.min(minInBinaryTree(root_add.left), minInBinaryTree(root_add.right)), root_add.val);
    }
};
...
public class Program_1
{
    ...
    public static void main(String[] args)
    {
        ...
        if(BT.createBinaryTree())
        {
            ...
            System.out.println("The minimum element in binary tree is:"+BT.minInBinaryTree(BT.root));
        }
        else
