...
/*Function to check whether children sum property exists in binary tree or not, that means for all nodes
except leaf node the value of current node equals the value of left child plus the value of right child*/
short unsigned int csp(struct TreeNode** root_add)
{
    ...
    else
    {
        ...
        if((*root_add)->left) leftChild=(*root_add)->left->val;
        if((*root_add)->right) rightChild=(*root_add)->right->val;
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        if(csp(&root)) printf("Children sum property exists in binary tree!\n");
        else printf("Children sum property does not exist in binary tree!\n");
