...
class BinaryTree{
    ...
    /*Member function of class: to check whether children sum property exists in binary tree or not, that means for all
    nodes except leaf node the value of current node equals the value of left child plus the value of right child*/
    boolean csp(TreeNode root_add)
    {
        /*Declaring base case for recursion: when the current node is a leaf node*/
        if(root_add.left==null && root_add.right==null) return true;
        else
        {
            /*Declaring two variables to store the values of left and right child of the
            current node and initializing them with zero in case any child node doesn't exist*/
            int leftChild=0, rightChild=0;

            /*Checking whether left subtree exists or not if it does then assigning it's value to leftChild and
            doing the same for right subtree and then checking whether their summation equals current node value*/
            if(root_add.left!=null) leftChild=root_add.left.val;
            if(root_add.right!=null) rightChild=root_add.right.val;

            /*Checking whether the summation of left and right child nodes equals current node's value or not
            if yes then we are going to call the function recursively for the left and right subtrees else no*/
            return root_add.val==leftChild+rightChild?(boolean)(csp(root_add.left) && csp(root_add.right)):false;
        }
    }
};
...
public class Program_1
{
    ...
    public static void main(String[] args)
    {
        ...
        if(BT.createBinaryTree())
        {
            ...
            if(BT.csp(BT.root)) System.out.println("Children sum property exists in binary tree!");
            else System.out.println("Children sum property does not exist in binary tree!");
        }
        else
