...
/*Function to find the maximum of two long long unsigned integers*/
long long unsigned int max(long long unsigned int a, long long unsigned int b) {return a>=b?a:b;}

/*Function to find the absolute value of difference of two long long unsigned integers*/
long long unsigned int llAbs(long long unsigned int a, long long unsigned int b) {return a>=b?(a-b):(b-a);}

/*Function to check whether binary tree is height balanced or not, that means whether the difference
between heights of left and right subtree for a current node is less than or equal to one or not*/
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        if(heightBalancedBT(&root)==ULLONG_MAX) printf("Binary tree is not height balanced!\n");
        else printf("Binary tree is height balanced!\n");
