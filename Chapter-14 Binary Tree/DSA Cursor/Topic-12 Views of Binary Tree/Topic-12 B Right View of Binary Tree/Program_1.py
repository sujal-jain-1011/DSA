...
and we are going to print the right view of the binary tree using recursive approach"""
...
class BinaryTree:
        ...
        def __init__(self):
            self.capacity, self.front, self.rear, self.visitedLevel=0, -1, -1, 0
        ...
        """Member function of class: to print the right view of the binary tree using recursive approach, for
        this we are going to use a class variables to keep track of visited level and current level"""
        def rightView(self, root_add, currentLevel):
            #Declaring base case for recursion: when binary tree is/becomes empty
            if(root_add):
                """Checking whether the current level is already visited or not, if not then we are go-
                ing to print the value of the current node and assign current level to visited level"""
                if(currentLevel>self.visitedLevel):
                    #Printing the current node's value and assigning currentLevel to visitedLevel
                    print(root_add.val, end=" ")
                    self.visitedLevel=currentLevel

                """Calling rightView function recursively for right and left subtrees of current node
                with the value of currentLevel increased by one, since we are traversing downwards"""
                self.rightView(root_add.right, currentLevel+1)
                self.rightView(root_add.left, currentLevel+1)
...
if(BT.createBinaryTree()):
    ...
    print("The right view of binary tree is:")
