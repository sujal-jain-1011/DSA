...
/*Function to print the right view of the binary tree using iterative approach*/
void rightView(struct TreeNode** root_add)
{
    ...
    else
    {
        ...
        else
        {
            ...
            int array[capacity], iter=0;
            enqueue((*root_add), &queLOT, &frontLOT, &rearLOT);
            enqueue(NULL, &queLOT, &frontLOT, &rearLOT);
            while(!isEmpty(frontLOT, rearLOT))
            {
                /*Getting and removing the front element of the queue, inserting elements of a level
                into array and enqueuing it's child nodes into the queue data structure, if it's not NULL*/
