...
and we are going to print the top view of the binary tree using recursive approach*/

/*Including iostream header file for input and output functions, limits.h header file for using ULLONG_MAX, map to use ordered_map, new
header file for memory allocation functions, queue file for using queue data structure and utility file for using pair data structure*/
#include<iostream>
#include<limits.h>
#include<map>
#include<new>
...
class BinaryTree{
    ...
    public:
        ...
        /*Member function of class: to print the top view of the binary tree using iterative approach*/
        void topView(struct TreeNode** root_add)
        {
            ...
            else
            {
                ...
                queue<pair<struct TreeNode*, long long>> queLOT;
                queLOT.push({*root_add, 0});
                ...
                while(!queLOT.empty())
                {
                    ...
                    pair<struct TreeNode*, long long> pr=queLOT.front(); queLOT.pop();
                    if(pr.first)
                    {
                        /*Enqueuing child nodes of the current node into queue if they exist*/
                        if(pr.first->left) queLOT.push({pr.first->left, pr.second-1});
                        if(pr.first->right) queLOT.push({pr.first->right, pr.second+1});

                        /*Checking whether current level already exists in map or not*/
                        if(hmap.find(pr.second)==hmap.end())
                        {
                            /*Inserting level variable as key and corresponding node value into map*/
                            hmap[pr.second]=pr.first->val;
                        }
                    }
                }
