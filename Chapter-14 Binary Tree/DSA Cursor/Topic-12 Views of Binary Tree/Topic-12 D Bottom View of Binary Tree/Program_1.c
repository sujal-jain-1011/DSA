...
/*Function to print the bottom view of the binary tree using iterative approach*/
void bottomView(struct TreeNode** root_add)
{
    ...
    else
    {
        ...
        else
        {
            ...
            long long signed int level=0; long long unsigned int iter=0;
            struct TreeNode* rootNode=*root_add;
            ...
            {
                /*Getting and removing the front element of the queue, inserting horizontal levels of tree along with,
            ...
            /*Populating an array from keys of the map and then sorting that array on the basis of keys*/
            size_t sz=(size_t)map_size(hmap), i=0;
            void *keys[sz]; map_get_keys(hmap, keys);
            long long sorted_keys[sz];
            for(size_t i=0; i<sz; i++) sorted_keys[i]=*(long long *)keys[i];
            qsort(sorted_keys, sz, sizeof(long long int), cmpfunc);
            ...
            {
                int *idx; map_get(hmap, sorted_keys+i, sizeof(sorted_keys[i]), (void**)&idx);
                printf("%d ", *idx);
            }
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        printf("The bottom view of the binary tree is:\n");
        bottomView(&root);
        printf("\n");
