...
/*Defining the map_size function: function for getting the size of GenericMap*/
size_t map_size(GenericMap *map) {return HASH_COUNT(map);}

/*Defining the map_get_keys function: function for getting the keys from GenericMap*/
void map_get_keys(GenericMap *map, void **keys_out)
{
    /*Iterating the GenericMap to get the keys from it*/
    GenericMap *curr, *tmp; size_t i = 0;
    HASH_ITER(hh, map, curr, tmp) {keys_out[i++] = curr->key;}
}
