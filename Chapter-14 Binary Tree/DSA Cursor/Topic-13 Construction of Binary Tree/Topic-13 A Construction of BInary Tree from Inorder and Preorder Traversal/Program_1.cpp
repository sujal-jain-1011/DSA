...
and output functions, new header file for memory allocation functions and unordered_map*/
#include<limits.h>
#include<iostream>
#include<new>
...
class BinaryTree{
    ...
    public:
        ...
        long long unsigned int capacity, front=ULLONG_MAX, rear=ULLONG_MAX, visitedLevel=0;
        ...
        struct TreeNode* helperCreateBT(unordered_map<int, long long unsigned int>& hmap, long long unsigned int low,
                                        long long unsigned int high, int* in, int* pre, long long unsigned int& t)
        {
            ...
            if(low<=high && t<capacity)
            {
                ...
                unsigned long long idx=hmap[pre[t]];
                struct TreeNode* r=createTreeNode(pre[t++]);
                ...
                r->left=idx>0?helperCreateBT(hmap, low, idx-1, in, pre, t):nullptr;
        ...
        struct TreeNode* createBT()
        {
            ...
            if(!pre) {cout<<"Memory allocation failed for preorder traversal vector!"<<endl; return nullptr;}
            ...
            for(long long unsigned int i=0; i<capacity; i++) {cout<<"Enter element "<<i<<" of preorder traversal vector:"; cin>>pre[i];}
            ...
            return helperCreateBT(hmap, 0, capacity-1, in, pre, iter);
        }
...
int main(void)
{
    ...
    if(BT.createBinaryTree()) cout<<"Successfully created binary tree!"<<endl;
