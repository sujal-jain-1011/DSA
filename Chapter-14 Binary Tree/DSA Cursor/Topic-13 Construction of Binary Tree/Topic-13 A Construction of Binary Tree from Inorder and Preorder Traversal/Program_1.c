...
/*Function to print the postorder traversal the binary tree by taking the address of root node*/
void postorder(struct TreeNode** root_add)
{
...
/*Helper function to build binary tree recursively from inorder and preorder traversal vector*/
struct TreeNode* helperCreateBT(GenericMap* hmap, long long unsigned int low, long long unsigned int high, int* in, int* pre, long long unsigned int* t)
{
    /*Declaring base case for recursion: when low exceeds high*/
    if(low<=high && (*t)<capacity)
    {
        ...
        unsigned long long *idx; map_get(hmap, pre+(*t), sizeof(pre[*t]), (void**)&idx);
        struct TreeNode* r=createTreeNode(pre[*t]); (*t)+=1;
        ...
        r->left=(*idx)>0?helperCreateBT(hmap, low, (*idx)-1, in, pre, t):NULL;
        r->right=helperCreateBT(hmap, (*idx)+1, high, in, pre, t);
...
/*Function to create binary tree from given inorder and preorder traversal vector
by taking input from user for the inorder and preorder traversal vectors*/
struct TreeNode* createBT()
{
    /*Declaring two pointers to integer to store the inorder and preorder traversal vectors,
    ...
    pre=(int*)malloc(capacity*sizeof(int));
    ...
    if(!pre) {printf("Memory allocation failed for preorder traversal vector!\n"); return NULL;}
    ...
    for(long long unsigned int i=0; i<capacity; i++) {printf("Enter element %llu of preorder traversal vector:", i); scanf("%d", pre+i);}
    ...
    struct TreeNode* new_root=helperCreateBT(hmap, 0, capacity-1, in, pre, &iter);
    
    /*Deallocating the dynamically allocated memory to in and pre pointers and retur-
