...
are also going to construct a binary tree from given inorder and postorder traversal vectors*/

/*Including generic_map.h header file for using custom map, limits.h header file for using ULLONG_MAX,
stdio.h header file for input and output functions and stdlib.h header file for memory allocation functions*/
...
/*Helper function to build binary tree recursively from inorder and postorder traversal vector*/
struct TreeNode* helperCreateBT(GenericMap* hmap, long long unsigned int low, long long unsigned int high, int* in, int* post, long long unsigned int* t)
{
    /*Declaring base case for recursion: when low exceeds high*/
    if(low<=high && (*t)!=ULLONG_MAX)
    {
        /*Getting the index of current element of postorder vector in inorder traversal vector and creating the root
        node for the current element and linking the root node with the corresponding right and left subtrees*/
        unsigned long long *idx; map_get(hmap, post+(*t), sizeof(post[*t]), (void**)&idx);
        struct TreeNode* r=createTreeNode(post[*t]); (*t)-=1;
        
        /*Calling helperCreateBT for right and left subtrees after doing a safety
        check on idx variable to avoid wrapping around of the idx variable*/
        r->right=helperCreateBT(hmap, (*idx)+1, high, in, post, t);
        r->left=(*idx)>0?helperCreateBT(hmap, low, (*idx)-1, in, post, t):NULL;
...
/*Function to create binary tree from given inorder and postorder traversal vector
by taking input from user for the inorder and postorder traversal vectors*/
struct TreeNode* createBT()
{
    /*Declaring two pointers to integer to store the inorder and postorder traversal vectors,
    allocating memory to these pointer variables dynamically and checking for memory allocation*/
    int* in, *post;
    in=(int*)malloc(capacity*sizeof(int));
    post=(int*)malloc(capacity*sizeof(int));

    /*Checking whether the memory allocation failed or not for in and post pointers*/
    if(!in) {printf("Memory allocation failed for inorder traversal vector!\n"); return NULL;}
    if(!post) {printf("Memory allocation failed for postorder traversal vector!\n"); return NULL;}

    /*Taking user input for inorder and postorder traversal vectors*/
    for(long long unsigned int i=0; i<capacity; i++) {printf("Enter element %llu of inorder traversal vector:", i); scanf("%d", in+i);}
    for(long long unsigned int i=0; i<capacity; i++) {printf("Enter element %llu of postorder traversal vector:", i); scanf("%d", post+i);}
    ...
    GenericMap *hmap=map_create();
    for(long long unsigned int i=0; i<capacity; i++) map_put(&hmap, in+i, sizeof(in[i]), &i, sizeof(i));
    ...
    long long unsigned int iter=capacity-1;
    struct TreeNode* new_root=helperCreateBT(hmap, 0, capacity-1, in, post, &iter);
    
    /*Deallocating the dynamically allocated memory to in and post pointers and retur-
    ning the newly created root node as the root node of newly created binary tree*/
    map_free(hmap);
    free(in); free(post);
    return new_root;
}
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        /*Printing the success message and deallocating memory to created
        binary tree and the queue used for creating the binary tree*/
    ...
    /*Calling createBT function to create binary tree from inorder and postorder traversal vectors
    and then the preorder function to perform the preorder traversal of the binary tree*/
    root=createBT();
    printf("The preorder traversal of the binary tree is:\n");
    preorder(&root);
    printf("\n");

    /*Deallocating memory to newly created binary tree*/
    freeBinaryTree(&root);
    return 0;
}