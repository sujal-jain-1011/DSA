...
and output functions and new header file for memory allocation functions and unordered_map*/
#include<limits.h>
#include<iostream>
#include<new>
...
class BinaryTree{
    ...
    public:
        ...
        /*Helper member function of class: to build binary tree recursively from inorder and postorder traversal vector*/
        struct TreeNode* helperCreateBT(unordered_map<int, long long unsigned int>& hmap, long long unsigned int low,
                                        long long unsigned int high, int* in, int* post, long long unsigned int& t)
        {
            ...
            if(low<=high && t!=ULLONG_MAX)
            {
                ...
                unsigned long long idx=hmap[post[t]];
                struct TreeNode* r=createTreeNode(post[t--]);
                ...
                r->left=idx>0?helperCreateBT(hmap, low, idx-1, in, post, t):nullptr;
        ...
        /*Member function of class: to create binary tree from given inorder and postorder traversal
        vector by taking input from user for the inorder and postorder traversal vectors*/
        struct TreeNode* createBT()
        {
            ...
            int* in, *post;
            in=new(nothrow) int[capacity];
            post=new(nothrow) int[capacity];

            /*Checking whether the memory allocation failed or not for in and post pointers*/
            if(!in) {cout<<"Memory allocation failed for inorder traversal vector!"<<endl; return nullptr;}
            if(!post) {cout<<"Memory allocation failed for postorder traversal vector!"<<endl; return nullptr;}

            /*Taking user input for inorder and postorder traversal vectors*/
            for(long long unsigned int i=0; i<capacity; i++) {cout<<"Enter element "<<i<<" of inorder traversal vector:"; cin>>in[i];}
            for(long long unsigned int i=0; i<capacity; i++) {cout<<"Enter element "<<i<<" of postorder traversal vector:"; cin>>post[i];}
            ...
            unordered_map<int, long long unsigned int> hmap;
            for(long long unsigned int i=0; i<capacity; i++) hmap[in[i]]=i;
            ...
            return helperCreateBT(hmap, 0, capacity-1, in, post, iter);
        }
