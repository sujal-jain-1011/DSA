...
class BinaryTree{
    ...
    int capacity, front=-1, rear=-1, iter;
    ...
    boolean createBinaryTree()
    {
        ...
        capacity=sc.nextInt(); iter=capacity-1;
    ...
    TreeNode helperCreateBT(Map<Integer, Integer> hmap, int low, int high, int[] in, int[] post)
    {
        ...
        if(low<=high && iter>=0)
        {
            ...
            int idx=hmap.get(post[iter]);
            TreeNode r=createTreeNode(post[iter--]);
            ...
            r.right=helperCreateBT(hmap, idx+1, high, in, post);
            r.left=idx>0?helperCreateBT(hmap, low, idx-1, in, post):null;
    ...
    TreeNode createBT()
    {
        ...
        /*Declaring two pointers to integer/arrays/list to store the inorder and postorder traversal vectors,
        ...
        try {post=new int[capacity];}
        catch(OutOfMemoryError e)
        {
            /*Printing the error message and returning*/
            System.err.println("Memory allocation failed for postorder vector!");
            return null;
        }
        ...
        for(int i=0; i<capacity; i++) {System.out.print("Enter element "+i+" of postorder traversal vector:"); post[i]=sc.nextInt();}
        ...
        return helperCreateBT(hmap, 0, capacity-1, in, post);
    }
};
