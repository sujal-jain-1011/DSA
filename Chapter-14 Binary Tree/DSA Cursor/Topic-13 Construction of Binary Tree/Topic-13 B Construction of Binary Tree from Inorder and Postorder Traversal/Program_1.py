...
class BinaryTree:
        ...
        def helperCreateBT(self, hmap, low, high, ino, post):
            #Declaring base case for recursion: when low exceeds high
            if(low<=high and self.it>=0):
                ...
                idx=hmap[post[self.it]]
                r=self.createTreeNode(post[self.it])
                self.it-=1
                ...
                r.right=self.helperCreateBT(hmap, idx+1, high, ino, post)
                r.left=self.helperCreateBT(hmap, low, idx-1, ino, post)
        ...
        def createBT(self):
            """Declaring two pointers to integer/arrays/list to store the inorder and postorder traversal
            vectors, allocating memory to these pointer variables dynamically and checking for memory
            allocation and taking user input for inorder and postorder traversal vectors"""
            ino=[input(f"Enter element {i} of inorder traversal vector:") for i in range(self.capacity)]
            post=[input(f"Enter element {i} of postorder traversal vector:") for i in range(self.capacity)]
            ...
            return self.helperCreateBT(hmap, 0, self.capacity-1, ino, post)
...
BT.root=BT.createBT()
print("The preorder traversal of the binary tree is:")
BT.preorder(BT.root)
print()
