...
long long unsigned int capacityArray[3];
long long unsigned int frontArray[3]={ULLONG_MAX, ULLONG_MAX, ULLONG_MAX};
long long unsigned int rearArray[3]={ULLONG_MAX, ULLONG_MAX, ULLONG_MAX};
...
short unsigned int implementCircularQueueForPair(Pair*** q)
{
    /*Allocating memory dynamically to create an array for implementing circular queue
    data structure and checking if the memory allocation was successful or not*/
    (*q)=(Pair**)malloc(*(capacityArray+2)*sizeof(Pair*));
    if((*q)==NULL)
    {
        printf("Memory allocation failed for queue data structure!\n");
        return 0;
...
void enqueuePair(Pair* pr, Pair*** q, long long unsigned int* front, long long unsigned int* rear, long long unsigned int *capacity)
{
    /*Checking whether the queue is full or not using the isFull function,
    if it is full then we can't insert and hence return without enqueuing*/
    if(isFull(front, rear, capacity))
    ...
    else {(*rear)=((*rear)+1)%(*capacity); (*q)[(*rear)]=pr;}
}
...
Pair* dequeuePair(Pair*** q, long long unsigned int* front, long long unsigned int* rear, long long unsigned int *capacity)
{
    /*Checking whether the queue is empty or not using the isEmpty function,
    if it is empty then we can't delete and hence return without dequeuing*/
    if(isEmpty(front, rear)) 
...
/*Function to find the maximum capacity from the capacities of binary trees*/
long long unsigned int maxCap(long long unsigned int a, long long unsigned int b) {return a>=b?a:b;}
...
short unsigned int isIdentical(struct TreeNode** root_add_1, struct TreeNode** root_add_2)
{
    ...
    *(capacityArray+2)=maxCap(*(capacityArray+0), *(capacityArray+1));
    ...
    else
    {
        ...
        enqueuePair(p, &queLOT, frontArray+2, rearArray+2, capacityArray+2);
        while(!isEmpty(frontArray+2, rearArray+2))
        {
            ...
            Pair* pr=dequeuePair(&queLOT, frontArray+2, rearArray+2, capacityArray+2);
            ...
            else
            {
                ...
                Pair *childPair_left=pair_create(&(leftChildPair_1), sizeof(leftChildPair_1), &(leftChildPair_2), sizeof(leftChildPair_2));
                Pair *childPair_right=pair_create(&(rightChildPair_1), sizeof(rightChildPair_1), &(rightChildPair_2), sizeof(rightChildPair_2));
                enqueuePair(childPair_left, &queLOT, frontArray+2, rearArray+2, capacityArray+2);
                enqueuePair(childPair_right, &queLOT, frontArray+2, rearArray+2, capacityArray+2);
            }
        }
...
int main(void)
{
    ...
    if(createBinaryTree(0) && createBinaryTree(1))
    {
        ...
        /*Calling isIdentical function to check whether binary trees are identical or not*/
        if(isIdentical(rootNodes+0, rootNodes+1)) printf("Binary trees are identical!\n");
        else printf("Binary trees are not identical!\n");
    }
