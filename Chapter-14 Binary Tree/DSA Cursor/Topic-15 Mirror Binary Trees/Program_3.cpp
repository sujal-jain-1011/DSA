...
/*Including limits.h header file for using ULLONG_MAX, iostream header file for input and output functions, new header
file for memory allocation functions queue file for using queue data structure and utility file for using pair data structure*/
#include<limits.h>
#include<iostream>
#include<new>
#include<queue>
...
class BinaryTree{
    ...
    public:
        ...
        bool isMirror(struct TreeNode** root_add_1, struct TreeNode** root_add_2)
        {
            ...
            while(!qu.empty())
            {
                ...
                pair<TreeNode*, TreeNode*> pr=qu.front(); qu.pop();
                struct TreeNode* qptr_1=pr.first;
                struct TreeNode* qptr_2=pr.second;
                ...
                else if(!qptr_1 || !qptr_2 || qptr_1->val!=qptr_2->val) return false;
                else
                {
                    ...
                    qu.push({qptr_1->left, qptr_2->right});
                    qu.push({qptr_1->right, qptr_2->left});
                }
            }
...
int main(void)
{
    ...
    if(BT.createBinaryTree(0) && BT.createBinaryTree(1))
    {
        ...
        /*Calling isMirror function to check whether binary trees are mirror images of each other or not*/
        if(BT.isMirror(BT.rootNodes+0, BT.rootNodes+1)) cout<<"Binary trees are mirror images!"<<endl;
        else cout<<"Binary trees are not mirror images!"<<endl;
    }
