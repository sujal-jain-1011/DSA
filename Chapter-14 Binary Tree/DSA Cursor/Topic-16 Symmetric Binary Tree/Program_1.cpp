...
class BinaryTree{
    ...
    public:
        ...
        {
            /*We are going to have four cases: when both nodes of the binary tree become NULL or empty, in that case we are
            going to return one since both of them are identical, else if when exactly one of the nodes of of the binary tree
            become NULL or empty in that case we are going to return zero since the binary tree is not symmetric about root
            node else when both of them are not NULL or non empty we are going to check for the remaining parts of tree*/
            if(!(*root_add_1) && !(*root_add_2)) return 1;
            else if(!(*root_add_1) || !(*root_add_2) || (*root_add_1)->val!=(*root_add_2)->val) return 0;
            else return isSymmetric(&((*root_add_1)->left), &((*root_add_2)->right)) && isSymmetric(&((*root_add_1)->right), &((*root_add_2)->left));
        }
...
int main(void)
{
    ...
    if(BT.createBinaryTree())
    {
        ...
        if(BT.isSymmetric(&(BT.root->left), &(BT.root->right))) cout<<"Binary tree is Symmetric!"<<endl;
        else cout<<"Binary tree is not symmetric!"<<endl;
