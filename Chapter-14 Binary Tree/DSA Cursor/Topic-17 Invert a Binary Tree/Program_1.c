...
/*Function to swap two pointers of the type (struct TreeNode*)*/
void swap(struct TreeNode** ptr_1, struct TreeNode** ptr_2)
{
    /*Swapping the pointer values using a third variable*/
    struct TreeNode *tempPtr=*ptr_1; *ptr_1=*ptr_2; *ptr_2=tempPtr;
}

/*Function to invert the binary tree that means swap left and right pointers of a node*/
void invert(struct TreeNode** root_add)
{
    /*Declaring base case for recursion: when binary tree is/becomes empty*/
    if((*root_add))
    {
        /*Swapping the left and right pointers of the current node and calling inv-
        ert function recursively for left and right subtree of the current node*/
        swap(&((*root_add)->left), &((*root_add)->right));
        invert(&((*root_add)->left));
        invert(&((*root_add)->right));
    }
}
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        printf("The preorder traversal of binary tree before inversion is:\n");
        preorder(&root); printf("\n");
        
        /*Calling insvert function to invert the binary tree*/
        invert(&root);
        printf("The preorder traversal of binary tree after inversion is:\n");
        preorder(&root); printf("\n");
