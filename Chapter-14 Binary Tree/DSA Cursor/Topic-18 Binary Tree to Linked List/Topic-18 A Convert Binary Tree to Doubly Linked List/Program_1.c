...
/*Function to traverse the doubly linked list that means printing the node value and previous and next addresses*/
void DLLTraversal(struct TreeNode* headPtr)
{
    ...
    while(headPtr)
    {
        ...
        printf("The address of previous node is:%p\n", headPtr->left);
...
/*Function to convert a binary tree to a doubly linked list using recursive approach*/
Pair* btToDLL(struct TreeNode** root_add)
{
    ...
    if((*root_add))
    {
        ...
        Pair* leftPair=btToDLL(&((*root_add)->left));
        struct TreeNode* leftHead=*(struct TreeNode**)(leftPair->first);
        struct TreeNode* leftTail=*(struct TreeNode**)(leftPair->second);
        ...
        if(!leftHead) leftHead=(*root_add); (*root_add)->left=leftTail;
        if(leftTail) leftTail->right=(*root_add); else leftTail=(*root_add);

        /*Calling the function recursively for right subtree of current node and getting the output returned
        by left call into a pointer to a custom pair and extracting it's contents into two pointers*/
        Pair* rightPair=btToDLL(&((*root_add)->right));
        struct TreeNode* rightHead=*(struct TreeNode**)(rightPair->first);
        struct TreeNode* rightTail=*(struct TreeNode**)(rightPair->second);

        /*Checking whether the extracted pointers require modifications or not
        and then linking them to the current node to form linked list structure*/
        if(!rightTail) rightTail=(*root_add); (*root_add)->right=rightHead;
        if(rightHead) rightHead->left=(*root_add); else rightHead=(*root_add);

        /*Returning the pair containing the head and tail of doubly linked list of current subtree*/
        Pair* retValue=pair_create(&leftHead, sizeof(leftHead), &rightTail, sizeof(rightTail));
        return retValue;
    }
    else
    {
        /*Returning a pair of NULL pointers if the current node does not exist*/
        struct TreeNode* dummyEmpty_1=NULL, *dummyEmpty_2=NULL;
        Pair* retValue=pair_create(&dummyEmpty_1, sizeof(dummyEmpty_1), &dummyEmpty_2, sizeof(dummyEmpty_2));
        return retValue;
    }
}

/*Function to deallocate the space allocated to binary tree(DLL) in order to avoid memory leakages*/
void freeDLL(struct TreeNode** root_add)
{
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        Pair* headPair=btToDLL(&root); root=*(struct TreeNode**)(headPair->first);
        printf("The traversal of doubly linked list is:\n");
        DLLTraversal(root);
