...
/*Function to traverse the singly linked list that means printing the node value and the address of next node*/
...
/*Function to convert a binary tree to a singly linked list using recursive approach*/
...
/*Function to deallocate the space allocated to binary tree(DLL) in order to avoid memory leakages*/
void freeSLL(struct TreeNode** root_add)
{
    ...
    while(next) {prev=next; next=next->right; free(prev);}
}
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        printf("The preorder traversal of binary tree before conversion is:\n");
        ...
        btToSLL(&root);
        printf("The traversal of singly linked list is:\n");
        SLLTraversal(root);
