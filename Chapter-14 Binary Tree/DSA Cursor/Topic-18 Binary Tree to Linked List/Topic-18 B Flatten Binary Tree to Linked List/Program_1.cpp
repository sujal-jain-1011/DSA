...
we are going to convert the binary tree into a singly linked list with same structure*/
...
class BinaryTree{
    ...
    public:
        ...
        /*Member function of class: to traverse the singly linked list that means printing the node value and the address of next node*/
        void SLLTraversal(struct TreeNode* headPtr)
        {
            /*Printing the value and address of current node and address of next nodes*/
            while(headPtr)
            {
                /*Printing the value and address of current node*/
                printf("The value of current node is:%d\n", headPtr->val);
                printf("The address of current node is:%p\n", headPtr);

                /*Printing the address of next node*/
                printf("The address of next node is:%p\n", headPtr->right);
                printf("\n");
        ...
        /*Member function of class: to convert a binary tree to a singly linked list using recursive approach*/
        struct TreeNode* btToSLL(struct TreeNode** root_add)
        {
            /*Declaring base case for recursion: when binary tree is/becomes empty*/
            if((*root_add))
            {
                /*We are going to call the btToSLL function recursively for left and right subtrees which  will
                return their respective tail pointers: pointing to their respective last nodes after conversion*/
                struct TreeNode* tailLeft=btToSLL(&((*root_add)->left));
                struct TreeNode* tailRight=btToSLL(&((*root_add)->right));

                /*Assigning left child to right field of current node and linking right child of current
                node to the right of tail of left subtree and returning tail of right subtree to the caller*/
                if(tailLeft)
                {
                    /*Linking right child of current node to right of tail of left subtree if it exists*/
                    tailLeft->right=(*root_add)->right;
                    (*root_add)->right=(*root_add)->left;
                }
                /*Assigning null pointer to left of current node*/
                (*root_add)->left=nullptr;
                
                /*Returning the tail pointer of the right subtree of the current node if it exists otherwise returning
                tail of left subtree and else if it doesn't exist then we are going to return pointer to current node*/
                if(!tailLeft && !tailRight) return (*root_add);
                else if(tailLeft && !tailRight) return tailLeft;
                else return tailRight;
            }
            else return NULL;
        }

        /*Member function of class: to deallocate the space allocated to binary tree(DLL) in order to avoid memory leakages*/
        void freeSLL(struct TreeNode** root_add)
        {
            /*Deallocating the allocated space using the free funtion*/
            struct TreeNode* prev, *next=(*root_add);
            while(next) {prev=next; next=next->right; delete prev;}
        }
};
...
int main(void)
{
    ...
    if(BT.createBinaryTree())
    {
        ...
        cout<<"The preorder traversal of binary tree before conversion is:"<<endl;
        BT.preorder(&(BT.root));
        cout<<endl;

        /*Calling btToDLL function to convert binary tree to singly linked list and
        then calling SLLTraversal function to print the traversal of linked list*/
        BT.btToSLL(&(BT.root));
        cout<<"The traversal of singly linked list is:"<<endl;
        BT.SLLTraversal(BT.root);
