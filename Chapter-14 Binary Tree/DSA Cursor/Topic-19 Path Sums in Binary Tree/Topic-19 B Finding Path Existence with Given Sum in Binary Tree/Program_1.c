...
long long unsigned int capacity, front=ULLONG_MAX, rear=ULLONG_MAX, isFound=0;
...
/*Function to check whether a path exists in the binary tree with given sum or not*/
void isPathExist(struct TreeNode** root_add, int targetSum, int sum)
{
    ...
    if((*root_add) && !isFound)
    {
        ...
        sum+=(*root_add)->val;
        if(!(*root_add)->left && !(*root_add)->right && sum==targetSum) {isFound=1; return;}
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        isPathExist(&root, targetSum, 0);

        /*Checking whether the path exists or not*/
        if(isFound) printf("Path exists in the binary tree with given sum!\n");
