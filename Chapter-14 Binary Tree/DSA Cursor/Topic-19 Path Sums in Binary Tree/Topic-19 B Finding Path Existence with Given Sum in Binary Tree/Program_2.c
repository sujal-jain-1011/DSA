...
and output functions, stdlib.h header file for memory allocation functions and pair.h
for using custom pair for storing pointer to TreeNode and an integer value as group*/
#include<limits.h>
#include<stdio.h>
#include<stdlib.h>
#include "pair.h"
...
/*Function to check whether root to leaf path with given sum exists or not*/
unsigned short isPathExist(struct TreeNode** root_add, int targetSum)
{
    /*Declaring queue data structure to store the node values along with the summation of nodes from root to
    that node in the form of pair and whenever we encounter a leaf node we are going to check for equality*/
    queLOT=(Pair**)malloc(capacity*sizeof(Pair*));
    if(!queLOT){printf("Circular queue creation failed!\n"); return 0;}
    else
    {
        /*Creating a pair which will contain root node value and the summation upto that node*/
        Pair* prRoot=pair_create((void*)(*(root_add)), sizeof(struct TreeNode), &((*(root_add))->val), sizeof(int));

        /*Enqueuing pair of TreeNode and vector of paths into queue data structure and
        iterating the queue to check which path nodes's summation equals targetSum*/
        ...
        while(!isEmpty(frontLOT, rearLOT))
        {
            ...
            front element contains leaf node or not if yes then checking for equality*/
            Pair* qr=dequeuePair(&queLOT, &frontLOT, &rearLOT);
            struct TreeNode* currentNode=(struct TreeNode*)(qr->first);
            int sum=*(int*)(qr->second);

            /*Checking whether the current node is a leaf node or not if yes then checking and returning one*/
            if(!(currentNode->left) && !(currentNode->right) && sum==targetSum) return 1;
            ...
            if(currentNode->left)
            {
                ...
                int leftSum=(currentNode->left)->val+sum;
                Pair* lc=pair_create((void*)(currentNode->left), sizeof(struct TreeNode), &(leftSum), sizeof(int));
                enqueuePair(lc, &queLOT, &frontLOT, &rearLOT);
            }
            if(currentNode->right)
            {
                /*Creating pair for holding left child of the current node along with updated sum*/
                int rightSum=(currentNode->right)->val+sum;
                Pair* rc=pair_create((void*)(currentNode->right), sizeof(struct TreeNode), &(rightSum), sizeof(int));
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        if(isPathExist(&root, targetSum)) printf("Path exists in the binary tree with given sum!\n");
        else printf("Path does not exist in the binary tree with given sum!\n");
