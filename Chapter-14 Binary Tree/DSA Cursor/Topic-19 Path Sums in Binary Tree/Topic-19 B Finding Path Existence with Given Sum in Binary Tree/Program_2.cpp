...
_MAX, new header file for memory allocation functions, queue for using the queue data structure and
utitlity for using pair data structure for storing pointer to TreeNode and an integer as a group*/
#include<iostream>
#include<limits.h>
#include<new>
#include<queue>
...
class BinaryTree{
    ...
    public:
        ...
        /*Member function of class: to check whether root to leaf path with given sum exists or not*/
        bool isPathExist(struct TreeNode** root_add, int targetSum)
        {
            /*Declaring queue data structure to store the node values along with the summation of nodes from root to
            to that node in the form of pair and whenever we encounter a leaf node we are going check for equality*/
            queue<pair<struct TreeNode*, int>> qu;
            ...
            while(!qu.empty())
            {
                ...
                pair<struct TreeNode*, int> qr=qu.front(); qu.pop();
                struct TreeNode* currentNode=qr.first; int sum=qr.second;

                /*Checking whether the current node is a leaf node or not if yes then checking and returning true*/
                if(!(currentNode->left) && !(currentNode->right) && sum==targetSum) return true;
                ...
                if(currentNode->left)
                {
                    ...
                    int leftSum=(currentNode->left)->val+sum; qu.push({currentNode->left, leftSum});
                }
                if(currentNode->right)
                {
                    /*Creating pair for holding right child of the current node along with updated sum*/
                    int rightSum=(currentNode->right)->val+sum; qu.push({currentNode->right, rightSum});
                }
            }

            /*Returning false if we come out of the loop, since no path exists with given targetSum*/
            return false;
        }
...
int main(void)
{
    ...
    if(BT.createBinaryTree())
    {
        ...
        if(BT.isPathExist(&(BT.root), targetSum)) cout<<"Path exists in the binary tree with given sum!"<<endl;
