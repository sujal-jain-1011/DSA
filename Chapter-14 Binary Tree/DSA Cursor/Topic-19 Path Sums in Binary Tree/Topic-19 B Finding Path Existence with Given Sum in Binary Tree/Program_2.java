...
class BinaryTree{
    ...
    boolean isPathExist(TreeNode root_add, int targetSum)
    {
        ...
        while(!qu.isEmpty())
        {
            ...
            Pair qr=qu.poll(); TreeNode currentNode=qr.ref; int sum=qr.sm;

            /*Checking whether the current node is a leaf node or not if yes then checking and returning true*/
            if((currentNode.left==null) && (currentNode.right==null) && sum==targetSum) return true;
            ...
            if(currentNode.left!=null)
            {
                ...
                int leftSum=(currentNode.left).val+sum; qu.offer(new Pair(currentNode.left, leftSum));
            }
            if(currentNode.right!=null)
            {
                /*Creating pair for holding right child of the current node along with updated sum*/
                int rightSum=(currentNode.right).val+sum; qu.offer(new Pair(currentNode.right, rightSum));
            }
        }
...
public class Program_2
{
    ...
    public static void main(String[] args)
    {
        ...
        if(BT.createBinaryTree())
        {
            ...
            if(BT.isPathExist(BT.root, targetSum)) System.out.println("Path exists in the binary tree with given sum!");
            else System.out.println("Path does not exist in the binary tree with given sum!");
        }
        else
