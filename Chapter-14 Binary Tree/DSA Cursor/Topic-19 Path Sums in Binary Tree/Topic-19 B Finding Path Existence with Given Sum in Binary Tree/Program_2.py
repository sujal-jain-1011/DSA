...
class BinaryTree:
        ...
        class Pair:
            ...
            def __init__(self, rootRef, summation):
                self.ref, self.sm=rootRef, summation
        ...
        def isPathExist(self, root_add, targetSum):
            ...
            qu.append(self.Pair(root_add, root_add.val))
            while(bool(qu)):
                ...
                qr=qu.popleft()
                currentNode, sum=qr.ref, qr.sm

                #Checking whether the current node is a leaf node or not if yes then checking and returning true
                if(not(currentNode.left) and not(currentNode.right) and sum==targetSum):
                    return True
                ...
                if(currentNode.left):
                    ...
                    leftSum=(currentNode.left).val+sum
                    qu.append(self.Pair(currentNode.left, leftSum))
                if(currentNode.right):
                    #Creating pair for holding right child of the current node along with updated sum
                    rightSum=(currentNode.right).val+sum
                    qu.append(self.Pair(currentNode.right, rightSum))
...
if(BT.createBinaryTree()):
    ...
    if(BT.isPathExist(BT.root, targetSum)):
        print("Path exists in the binary tree with given sum!")
    else:
        print("Path does not exist in the binary tree with given sum!")
else:
