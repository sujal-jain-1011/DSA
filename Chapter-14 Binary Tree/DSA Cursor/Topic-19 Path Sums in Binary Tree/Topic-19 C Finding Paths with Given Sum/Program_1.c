...
/*Function to print all the root to leaf paths in the binary tree with given sum*/
void printPaths(struct TreeNode** root_add, vector* path, int targetSum, int sum)
{
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        printPaths(&root, &vec, targetSum, 0);
        vecFree(&vec);
