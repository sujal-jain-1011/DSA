...
we are also going to find all the root to leaf paths in the binary tree with given sum*/

/*Including iostream header file for input and output functions, limits.h header file for using
ULLONG_MAX, new header file for memory allocation functions and vector file to use vectors*/
#include<iostream>
#include<limits.h>
#include<new>
#include<vector>
...
class BinaryTree{
    ...
    public:
        ...
        /*Member function of class: to print all the root to leaf paths in the binary tree with given sum*/
        void printPaths(struct TreeNode** root_add, vector<int>& path, int targetSum, int sum)
        {
            /*Declaring base case for recursion: when binary tree is/becomes empty*/
            if((*root_add))
            {
                /*Adding current node's value to sum and then pushing the data of the current node into vector and then calling the
                printPaths function recursively for left and right subtree of current node and then popping current element from vector*/
                sum+=(*root_add)->val;
                path.push_back((*root_add)->val);
                ...
                if(!(*root_add)->left && !(*root_add)->right && sum==targetSum)
                {
                    /*Printing the current contents of the vector and then returning after popping the current node*/
                    cout<<"Path "<<(1+pathCounter++)<<" of binary tree is: "; long long unsigned int sz=path.size();
                    for(long long unsigned int it=0; it<sz; it++) cout<<path[it]<<" ";
                    cout<<endl;
                ...
                /*Calling the printPaths function recursively for left and right subtree of current node*/
                printPaths(&((*root_add)->left), path, targetSum, sum);
                printPaths(&((*root_add)->right), path, targetSum, sum);
...
int main(void)
{
    ...
    if(BT.createBinaryTree())
    {
        ...
        /*Declaring a variable of the vector type(user defined), calling printPaths function to print all the
        paths of binary tree with given sum and calling vecFree function to free the allocated space to vector*/
        vector<int> vec;
        BT.printPaths(&(BT.root), vec, targetSum, 0);
