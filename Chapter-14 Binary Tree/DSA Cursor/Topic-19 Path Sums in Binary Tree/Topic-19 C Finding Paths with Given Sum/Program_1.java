...
/*Importing Scanner class for taking the input from user and Vector class for using vector from java.util*/
...
class BinaryTree{
    ...
    int capacity, front=-1, rear=-1, pathCounter=0;
    ...
    void printPaths(TreeNode root_add, int targetSum, int sum)
    {
        ...
        if(root_add!=null)
        {
            ...
            if(root_add.left==null && root_add.right==null && sum==targetSum)
            {
                /*Printing the current contents of the vector and then returning after popping the current node*/
                System.out.print("Path "+(1+pathCounter++)+" of binary tree is: "); int sz=path.size();
                for(int it=0; it<sz; it++) System.out.print(path.get(it)+" ");
                System.out.println();
            ...
            printPaths(root_add.left, targetSum, sum);
            printPaths(root_add.right, targetSum, sum);
...
public class Program_1
{
    ...
    public static void main(String[] args)
    {
        ...
        if(BT.createBinaryTree())
        {
            ...
            /*Calling printPaths function to print all the paths of binary tree with given sum*/
            BT.printPaths(BT.root, targetSum, 0);
        }
        else
