...
/*Function to print all the root to leaf paths with given target sum in binary tree*/
void allPaths(struct TreeNode** root_add, int targetSum)
{
    ...
    else
    {
        ...
        vecPush(&vecRoot, &((*(root_add))->val)); Pair* prRoot=pair_create((void*)(*(root_add)),
        sizeof(struct TreeNode), &vecRoot, sizeof(vector));
        ...
        while(!isEmpty(frontLOT, rearLOT))
        {
            ...
            vector current=*(vector*)(qr->second); int sum=*(int*)vecGet(&current, 0);
            ...
            if(!(currentNode->left) && !(currentNode->right) && sum==targetSum)
            {
                ...
                for(size_t iter=1; iter<sz; iter++) printf("%d ", *(int*)vecGet(&current, iter)); printf("\n");
            }
            ...
            if(currentNode->left)
            {
                ...
                vector leftChild; vecInit(&leftChild, sizeof(int)); int leftSum=*(int*)vecGet(&current, 0)+(currentNode->left)->val;
                vecPush(&leftChild, &leftSum); size_t sz=current.length;
                for(size_t iter=1; iter<sz; iter++) vecPush(&leftChild, (int*)vecGet(&current, iter));
            ...
            if(currentNode->right)
            {
                ...
                vector rightChild; vecInit(&rightChild, sizeof(int)); int rightSum=*(int*)vecGet(&current, 0)+(currentNode->right)->val;
                vecPush(&rightChild, &rightSum); size_t sz=current.length;
                for(size_t iter=1; iter<sz; iter++) vecPush(&rightChild, (int*)vecGet(&current, iter));
