...
_MAX, new header file for memory allocation functions, queue for using the queue data structure, vector
file for using dynamic array (array of variable size) and utitlity for using pair data structure*/
#include<iostream>
#include<limits.h>
#include<new>
#include<queue>
...
class BinaryTree{
    ...
    public:
        ...
        void allPaths(struct TreeNode** root_add, int targetSum)
        {
            ...
            queue<pair<struct TreeNode*, vector<int>>> qu;
            ...
            vector<int> rootVal; rootVal.push_back((*(root_add))->val); rootVal.push_back((*(root_add))->val);
            ...
            while(!qu.empty())
            {
                ...
                pair<struct TreeNode*, vector<int>> qr=qu.front(); qu.pop();
                struct TreeNode* currentNode=qr.first;
                vector<int> current((qr.second).begin(), (qr.second).end());
                int sum=current[0];

                /*Checking whether the current node is a leaf node or not if yes then printing after checking*/
                if(!(currentNode->left) && !(currentNode->right) && sum==targetSum)
                {
                    /*Printing the values of root to leaf path values stored in the associated vector*/
                    size_t sz=current.size(); cout<<"Path "<<(1+noOfPaths++)<<" of binary tree is: ";
                    for(size_t iter=1; iter<sz; iter++) cout<<current[iter]<<" "; cout<<endl;
                }
                ...
                if(currentNode->left)
                {
                    ...
                    vector<int> leftChild(current.begin(), current.end()); leftChild[0]+=(currentNode->left)->val;
                    leftChild.push_back((currentNode->left)->val); qu.push({currentNode->left, leftChild});
                }
                if(currentNode->right)
                {
                    /*Creating pair for holding right child of the current node along with updated path vector*/
                    vector<int> rightChild(current.begin(), current.end()); rightChild[0]+=(currentNode->right)->val;
                    rightChild.push_back((currentNode->right)->val); qu.push({currentNode->right, rightChild});
                }
            }
        }
...
int main(void)
{
    ...
    if(BT.createBinaryTree())
    {
        ...
        int targetSum;
        cout<<"Enter the target sum:"; cin>>targetSum;

        /*Calling allPaths function to print all the root to leaf paths with given target sum*/
