...
we are also going to print all the root to leaf paths with given target sum in BT*/
...
and List and ArrayList class for implementing vector (java based dynamic array)*/
...
class BinaryTree{
    ...
    /*Defining a custom class for holding reference of type TreeNode and List of integers*/
    class Pair{
        /*Defining data members of class: reference of TreeNode type, List of integers and constructor for initialization*/
        TreeNode ref; List<Integer> vec; Pair(TreeNode ref, List<Integer> vec){this.ref=ref; this.vec=vec;}
    }
    ...
    /*Member function of class: to print all the root to leaf paths with given target sum in binary tree*/
    void allPaths(TreeNode root_add, int targetSum)
    {
        /*Declaring a variable to keep track of root to leaf paths in the binary tree*/
        int noOfPaths=0;
        
        /*Declaring queue data structure to store the node values along with the vector containing path from root to that
        node in the form of pair and whenever we encounter a leaf node we are going to print that vector after checking*/
        ...
        List<Integer> rootVal=new ArrayList<>(); rootVal.add(root_add.val); rootVal.add(root_add.val);
        ...
        qu.offer(new Pair(root_add, rootVal));
        while(!qu.isEmpty())
        {
            ...
            Pair qr=qu.poll(); TreeNode currentNode=qr.ref;
            List<Integer> current=new ArrayList<>(qr.vec);
            int sum=current.get(0);

            /*Checking whether the current node is a leaf node or not if yes then printing after checking*/
            if((currentNode.left==null) && (currentNode.right==null) && sum==targetSum)
            {
                ...
                int sz=current.size(); System.out.print("Path "+(1+noOfPaths++)+" of binary tree is: ");
                for(int iter=1; iter<sz; iter++) System.out.print(current.get(iter)+" "); System.out.println();
            }
            ...
            if(currentNode.left!=null)
            {
                ...
                List<Integer> leftChild=new ArrayList<>(current); leftChild.set(0, leftChild.get(0)+(currentNode.left).val);
                leftChild.add((currentNode.left).val); qu.offer(new Pair(currentNode.left, leftChild));
            }
            if(currentNode.right!=null)
            {
                /*Creating pair for holding left child of the current node along with updated path vector*/
                List<Integer> rightChild=new ArrayList<>(current); rightChild.set(0, rightChild.get(0)+(currentNode.right).val);
                rightChild.add((currentNode.right).val); qu.offer(new Pair(currentNode.right, rightChild));
            }
        }
    }
};
...
public class Program_2
{
    ...
    public static void main(String[] args)
    {
        ...
        if(BT.createBinaryTree())
        {
            ...
            int targetSum; Scanner sc=new Scanner(System.in);
            System.out.print("Enter the target sum:"); targetSum=sc.nextInt();
            sc.close();

            /*Calling allPaths function to print all the root to leaf paths*/
            BT.allPaths(BT.root, targetSum);
        }
        else
