...
class BinaryTree:
        ...
        class Pair:
            ...
            def __init__(self, rootRef, vector):
                self.ref, self.vec=rootRef, vector
        ...
        def allPaths(self, root_add, targetSum):
            ...
            rootVal=[root_add.val, root_add.val]
            ...
            qu.append(self.Pair(root_add, rootVal))
            while(bool(qu)):
                ...
                qr=qu.popleft()
                currentNode, current, sum=qr.ref, list(qr.vec), qr.vec[0]
                ...
                if(not((currentNode.left)) and (not(currentNode.right)) and sum==targetSum):
                    ...
                    sz=len(current)
                    print(f"Path {1+noOfPaths} of binary tree is:", end=" ")
                    noOfPaths+=1
                    for iter in range(1, sz):
                ...
                if(currentNode.left):
                    ...
                    leftChild=list(current)
                    leftChild[0]+=(currentNode.left).val
                    leftChild.append((currentNode.left).val)
                    qu.append(self.Pair(currentNode.left, leftChild))
                if(currentNode.right):
                    #Creating pair for holding left child of the current node along with updated path vector
                    rightChild=list(current)
                    rightChild[0]+=(currentNode.right).val
                    rightChild.append((currentNode.right).val)
                    qu.append(self.Pair(currentNode.right, rightChild))
