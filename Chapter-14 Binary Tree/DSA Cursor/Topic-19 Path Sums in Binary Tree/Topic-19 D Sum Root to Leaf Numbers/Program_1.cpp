...
and we are also going to find sum of all numbers formed from path from root to leaf*/
...
class BinaryTree{
    ...
    public:
        ...
        /*Member function of class: to find the sum of all the numbers formed from path from root to leaf*/
        void sumRootToLeaf(struct TreeNode** root_add, int d)
        {
            /*Declaring base case for recursion: when binary tree is/becomes empty*/
            if((*root_add))
            {
                /*Increasing the place value of the received digit and adding current node value into the digit*/
                d=10*d+(*root_add)->val;

                /*Checking whether the current node is a leaf node or not, if it is leaf node
                then we are going to add current digit value into the totalSum o/w recursive calls*/
                if(!(*root_add)->left && !(*root_add)->right)
                {
                    /*Adding current digit value to totalSum and returning from here*/
                    totalSum+=d;
                    return;
                }

                /*Calling the printPaths function recursively for left and right subtree of current node*/
                sumRootToLeaf(&((*root_add)->left), d);
                sumRootToLeaf(&((*root_add)->right), d);
            }
        }
...
int main(void)
{
    ...
    if(BT.createBinaryTree())
    {
        ...
        /*Calling the sumRootToLeaf function to find the sum of all the numbers formed from path from root to leaf*/
        BT.sumRootToLeaf(&(BT.root), 0);
        cout<<"The sum of all the numbers formed from path from root to leaf is:"<<BT.totalSum<<endl;
