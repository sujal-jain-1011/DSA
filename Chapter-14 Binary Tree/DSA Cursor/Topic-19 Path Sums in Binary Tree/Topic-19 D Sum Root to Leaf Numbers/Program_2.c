...
/*Function to calculate the summation of all root to leaf path numbers of binary tree*/
int sumRootToLeaf(struct TreeNode** root_add)
{
    ...
    int accSum=0; queLOT=(Pair**)malloc(capacity*sizeof(Pair*));
    ...
    else
    {
        ...
        /*Enqueuing pair of TreeNode and vector of paths into queue data structure
        and iterating the queue to accumulate all root to leaf path nodes's summation*/
        ...
        while(!isEmpty(frontLOT, rearLOT))
        {
            ...
            if(currentNode->left)
            {
                ...
                int leftSum=(10*digit)+(currentNode->left)->val;
            ...
            {
                /*Creating pair for holding left child of the current node along with updated sum*/
                int rightSum=(10*digit)+(currentNode->right)->val;
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        printf("Summation of all root to leaf path numbers:%d\n", sumRootToLeaf(&root));
