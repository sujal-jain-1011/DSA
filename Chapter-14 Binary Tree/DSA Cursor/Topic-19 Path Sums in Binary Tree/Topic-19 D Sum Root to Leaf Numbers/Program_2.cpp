...
class BinaryTree{
    ...
    public:
        ...
        int sumRootToLeaf(struct TreeNode** root_add)
        {
            ...
            int accSum=0; queue<pair<struct TreeNode*, int>> qu;
            ...
            while(!qu.empty())
            {
                ...
                struct TreeNode* currentNode=qr.first; int digit=qr.second;
                ...
                if(currentNode->left)
                {
                    ...
                    int leftSum=(10*digit)+(currentNode->left)->val; qu.push({currentNode->left, leftSum});
                }
                if(currentNode->right)
                {
                    /*Creating pair for holding right child of the current node along with updated sum*/
                    int rightSum=(10*digit)+(currentNode->right)->val; qu.push({currentNode->right, rightSum});
                }
            }
...
int main(void)
{
    ...
    if(BT.createBinaryTree())
    {
        ...
        cout<<"Summation of all root to leaf path numbers:"<<BT.sumRootToLeaf(&(BT.root))<<endl;
