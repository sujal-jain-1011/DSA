...
and we are also going to find maximum path sum in the constructed binary tree*/
...
long long unsigned int capacity, front=ULLONG_MAX, rear=ULLONG_MAX; int totalSum=0;
...
/*Function to find the maximum of two integers and return the maximum*/
int max(int a, int b){return a>=b?a:b;}

/*Function to find the maximum path sum (not necessarily passing through root) in binary tree*/
int maxPathSum(struct TreeNode** root_add, int* mps)
{
    /*Declaring base case for recursion: when binary tree is/becomes empty*/
    if((*(root_add)))
    {
        /*Getting the maximum path sum from left and right subtrees of the current node, checking
        whether the path can be extended with the current node or not, updating mps variable and
        returning the maximum path from the subtree rooted at current node to the caller function*/
        int leftSum=maxPathSum(&((*(root_add))->left), mps), rightSum=maxPathSum(&((*(root_add))->right), mps);
        int firstMax=max((*(root_add))->val+leftSum, (*(root_add))->val+rightSum),
        secondMax=max((*(root_add))->val+leftSum+rightSum, (*(root_add))->val), thirdMax=max(firstMax, secondMax);

        /*Updating global mps variable and returning maximum path sum of subtree rooted at current node*/
        (*mps)=max((*mps), thirdMax); return max(firstMax, (*(root_add))->val);
    }
    else return 0;
}
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        /*Declaring a variable to store the maximum path sum of binary tree
        and calling maxPathSum to populate this variable from max path sum*/
        int mps=INT_MIN; maxPathSum(&root, &mps);
        printf("The maximum path sum in the binary tree is:%d\n", mps);
