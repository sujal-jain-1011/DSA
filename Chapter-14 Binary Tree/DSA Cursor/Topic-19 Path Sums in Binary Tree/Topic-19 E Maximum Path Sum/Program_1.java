...
class BinaryTree{
    ...
    int capacity, front=-1, rear=-1, totalSum=0;
    ...
    int maxPathSum(TreeNode root_add, int[] mps)
    {
        ...
        if(root_add!=null)
        {
            ...
            int firstMax=Math.max(root_add.val+leftSum, root_add.val+rightSum),
            secondMax=Math.max(root_add.val+leftSum+rightSum, root_add.val), thirdMax=Math.max(firstMax, secondMax);

            /*Updating global mps variable and returning maximum path sum of subtree rooted at current node*/
            mps[0]=Math.max(mps[0], thirdMax); return Math.max(firstMax, root_add.val);
        }
        else return 0;
    }
};
...
public class Program_1
{
    ...
    public static void main(String[] args)
    {
        ...
        if(BT.createBinaryTree())
        {
            ...
            int[] mps=new int[1]; mps[0]=Integer.MIN_VALUE; BT.maxPathSum(BT.root, mps);
            System.out.println("The maximum path sum in the binary tree is:"+mps[0]);
        }
        else
