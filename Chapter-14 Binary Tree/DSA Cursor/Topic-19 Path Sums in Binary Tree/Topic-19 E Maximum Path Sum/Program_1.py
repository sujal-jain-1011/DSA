...
#Importing math module for using negative infinity
import math
...
class BinaryTree:
        ...
        def __init__(self):
            self.capacity, self.front, self.rear, self.totalSum=0, -1, -1, 0
        ...
        def maxPathSum(self, root_add, mps):
            ...
            if(root_add):
                ...
                leftSum, rightSum=self.maxPathSum(root_add.left, mps), self.maxPathSum(root_add.right, mps)
                firstMax=max(root_add.val+leftSum, root_add.val+rightSum)
                secondMax=max(root_add.val+leftSum+rightSum, root_add.val)
                thirdMax=max(firstMax, secondMax)

                #Updating global mps variable and returning maximum path sum of subtree rooted at current node
                mps[0]=max(mps[0], thirdMax)
                return max(firstMax, root_add.val)
            else:
                return 0
...
if(BT.createBinaryTree()):
    ...
    mps=[-math.inf]
    BT.maxPathSum(BT.root, mps)
    print(f"The maximum path sum in the binary tree is:{mps[0]}")
else:
