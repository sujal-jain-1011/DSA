...
are also going to check whether one binary tree subtree of another binary tree or not*/
...
/*Function to check whether second binary tree is subtree of first binary tree or not*/
short unsigned int isSubtree(struct TreeNode** root_add_1, struct TreeNode** root_add_2)
{
    ...
    if(!(*(root_add_2))) return 1;
    else if(!(*(root_add_1))) return 0;
    else
    {
        ...
        if((*(root_add_1))->val==(*(root_add_2))->val)
        {
            ...
            if(isIdentical(root_add_1, root_add_2)) return 1;
            else
    ...
    /*Returning zero to avoid warnings*/
    return 0;
}
...
int main(void)
{
    ...
    if(createBinaryTree(0) && createBinaryTree(1))
    {
        ...
        if(isSubtree(rootNodes+0, rootNodes+1)) printf("Binary tree 2 is subtree of binary tree 1!\n");
        else printf("Binary tree 2 is not a subtree of binary tree 1!\n");
    }
