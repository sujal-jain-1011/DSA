...
file for using ULLONG_MAX, new header file for memory allocation functions, map
for using ordered map and vector for using dynamic (variable length) arrays*/
#include<iostream>
#include<limits.h>
#include<new>
#include<map>
...
/*Typedefining long long unsigned int to llui*/
...
class BinaryTree{
    ...
    public:
        ...
        void populateMap(struct TreeNode** root_add, map<llui, vector<struct TreeNode**>>& mp, llui currLevel)
        {
            ...
            if((*(root_add)))
            {
                ...
                mp[currLevel].push_back(root_add);
