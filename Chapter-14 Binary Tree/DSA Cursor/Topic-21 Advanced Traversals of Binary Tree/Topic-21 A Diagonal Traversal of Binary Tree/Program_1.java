...
Map, TreeMap for using ordered map to store key value pairs in order and
ArrayList, List to implement dynamic (variable length) arrays useful here*/
...
class BinaryTree{
    ...
    /*Member function of class: to populate the user defined map with the diagonal levels and values at those levels*/
    void populateMap(TreeNode root_add, Map<Integer, List<TreeNode>> mp, int currLevel)
    {
        /*Declaring base case for recursion: when binary tree is/becomes empty*/
        if(root_add!=null)
        {
            /*Inserting current node's address into the ordered map then calling the popula-
            teMap function recursively for the left and right subtree of current node*/
            mp.computeIfAbsent(currLevel, k->new ArrayList<>()).add(root_add);
            populateMap(root_add.left, mp, currLevel+1);
            populateMap(root_add.right, mp, currLevel);
        }
    }
};
...
public class Program_1
{
    ...
    public static void main(String[] args)
    {
        ...
        if(BT.createBinaryTree())
        {
            ...
            /*Calling populateMap function to populate an ordered map
            with key value pair containing the addresses of addresses
            of node values of binary tree of different diagonal levels*/
            Map<Integer, List<BinaryTree.TreeNode>> mp=new TreeMap<>();
            BT.populateMap(BT.root, mp, 0);

            /*Displaying a user friendly message for printing the diagonal order traversal*/
            System.out.println("The diagonal order traversal of binary tree is:");
            
            /*Iterating the map to print the diagonal order traversal*/
            for(Map.Entry<Integer, List<BinaryTree.TreeNode>> entry:mp.entrySet())
            {
                /*Printing the values of the different diagonal levels*/
                int sz=entry.getValue().size();
                for(int i=0; i<sz; i++) System.out.print(entry.getValue().get(i).val+" ");
            }
            System.out.println();
        }
        else
