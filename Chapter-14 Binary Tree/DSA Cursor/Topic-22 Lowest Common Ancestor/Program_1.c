...
/*Function to find the lowest common ancestor of two given unique nodes of the binary tree*/
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        int p, q; printf("Enter the node values:"); scanf("%d %d", &p, &q); struct TreeNode** ans=lca(&root, p, q);
        if(!ans) printf("The lowest common ancestor for %d and %d does not exist!\n", p, q);
        else printf("The lowest common ancestor for %d and %d is:%d\n", p, q, (*ans)->val);
