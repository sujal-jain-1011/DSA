...
class BinaryTree{
    ...
    public:
        ...
        struct TreeNode** lca(struct TreeNode** root_add, int p, int q)
        {
            ...
            if((*(root_add)))
            {
                ...
                struct TreeNode** leftLCA=lca(&((*(root_add))->left), p, q);
                struct TreeNode** rightLCA=lca(&((*(root_add))->right), p, q);

                /*Checking the value returned by the function calls for left and right subtree*/
                if(leftLCA && rightLCA) return root_add;
                else if(leftLCA || rightLCA) return (leftLCA)?leftLCA:rightLCA;
                else return nullptr;
            }
            else return nullptr;
        }
...
int main(void)
{
    ...
    if(BT.createBinaryTree())
    {
        ...
        int p, q; cout<<"Enter the node values:"; cin>>p>>q; BinaryTree::TreeNode** ans=BT.lca(&(BT.root), p, q);
        if(!ans) cout<<"The lowest common ancestor for "<<p<<" and "<<q<<" does not exist!"<<endl;
        else cout<<"The lowest common ancestor for "<<p<<" and "<<q<<" is:"<<(*ans)->val<<endl;
