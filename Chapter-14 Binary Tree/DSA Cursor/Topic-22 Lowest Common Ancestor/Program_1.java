...
we are also going to find the lowest common ancestor of two unique nodes of binary tree*/
...
class BinaryTree{
    ...
    /*Member function of class: to find the lowest common ancestor of two given unique nodes of the binary tree*/
    TreeNode lca(TreeNode root_add, int p, int q)
    {
        /*Declaring base case for recursion: when binary tree is/becomes empty*/
        if(root_add!=null)
        {
            /*Checking whether the value of current node equals any of the p, q value or not,
            if yes then we are going to return current nodde address otherwise we are going
            to call lca function recursively for the left and right subtree of current node*/
            if(root_add.val==p || root_add.val==q) return root_add;
            TreeNode leftLCA=lca(root_add.left, p, q);
            TreeNode rightLCA=lca(root_add.right, p, q);

            /*Checking the value returned by the function calls for left and right subtree*/
            if(leftLCA!=null && rightLCA!=null) return root_add;
            else if(leftLCA!=null || rightLCA!=null) return (leftLCA!=null)?leftLCA:rightLCA;
            else return null;
        }
        else return null;
    }
...
public class Program_1
{
    ...
    public static void main(String[] args)
    {
        ...
        if(BT.createBinaryTree())
        {
            ...
            /*Getting the user input for the nodes of whose lowest common ancestor we need
            to find out and calling lca function for the binary tree with the user values*/
            Scanner sc=new Scanner(System.in); int p, q; System.out.print("Enter the node values:"); 
            p=sc.nextInt(); q=sc.nextInt(); BinaryTree.TreeNode ans=BT.lca(BT.root, p, q);
            if(ans==null) System.out.println("The lowest common ancestor for "+p+" and "+q+" does not exist!");
            else System.out.println("The lowest common ancestor for "+p+" and "+q+" is:"+ans.val); sc.close();
        }
        else
