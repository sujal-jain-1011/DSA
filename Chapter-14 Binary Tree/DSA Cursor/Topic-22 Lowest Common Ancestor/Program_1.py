...
class BinaryTree:
        ...
        def lca(self, root_add, p, q):
            ...
            if(root_add):
                ...
                if(root_add.val==p or root_add.val==q):
                    return root_add
                leftLCA=self.lca(root_add.left, p, q)
                rightLCA=self.lca(root_add.right, p, q)

                #Checking the value returned by the function calls for left and right subtree
                if(leftLCA and rightLCA):
                    return root_add
                elif(leftLCA or rightLCA):
...
if(BT.createBinaryTree()):
    ...
    p, q=map(int, input("Enter the node values:").split())
    ans=BT.lca(BT.root, p, q)
    if(not(ans)):
        print(f"The lowest common ancestor for {p} and {q} does not exist!")
    else:
        print(f"The lowest common ancestor for {p} and {q} is:{ans.val}")
else:
