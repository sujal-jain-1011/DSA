...
and we are also going to find the diameter (length of longest path) of the binary tree*/
...
/*Function to find the maximum of two long long unsigned int*/
llui max(llui a, llui b){return a>=b?a:b;}

/*Function to find the diameter (length of longest path) of the binary tree*/
long long unsigned int diameter(struct TreeNode** root_add, long long unsigned int* diam)
{
    /*Declaring base case for recursion: when binary tree is/becomes empty*/
    if((*(root_add)))
    {
        /*Calling the diameter function recursively for left and right subtree of
        the current node for getting their heights and comparing with diameter*/
        long long unsigned int leftHeight=diameter(&((*root_add)->left), diam);
        long long unsigned int rightHeight=diameter(&((*root_add)->right), diam);

        /*Comparing summation of left and right subtree height with existing diameter
        and then returning the maximum of left and right subtree's height to caller*/
        (*(diam))=max((*diam), leftHeight+rightHeight+1);
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        /*Calling diameter function to find the diameter of binary tree*/
        llui diam=0; diameter(&root, &diam);
        if(diam==0) printf("The diameter of binary tree is:%llu\n", diam);
        else printf("The diameter of binary tree is:%llu\n", diam-1);
