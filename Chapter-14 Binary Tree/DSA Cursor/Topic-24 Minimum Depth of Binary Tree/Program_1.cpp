...
and we are also going to find the minimum depth of the given binary tree recursively*/
...
class BinaryTree{
    ...
    public:
        ...
        /*Member function of class: to find the minimum depth of given binary tree recursively*/
        long long unsigned int minDepth(struct TreeNode** root_add)
        {
            /*Declaring base case for recursion: when binary tree is/becomes empty*/
            if((*(root_add)))
            {
                /*Calling minDepth function recursively for the left and right subtree
                of the current node and then comparing them to get the minimum depth*/
                llui leftHeight=ULLONG_MAX, rightHeight=ULLONG_MAX;
                if(((*(root_add))->left)) leftHeight=minDepth(&((*(root_add))->left));
                if(((*(root_add))->right)) rightHeight=minDepth(&((*(root_add))->right));

                /*Handling the corner case: when both the subtrees of current node are null*/
...
int main(void)
{
    ...
    if(BT.createBinaryTree())
    {
        ...
        cout<<"The minimum depth of binary tree is:"<<BT.minDepth(&(BT.root))<<endl;
