    ...
    else
    {
        ...
        else
        {
            ...
            while(!isEmpty(frontLOT, rearLOT))
            {
                ...
                struct TreeNode* qptr=dequeue(&queLOT, &frontLOT, &rearLOT), *nptr=NULL;
                if(!isEmpty(frontLOT, rearLOT)) nptr=queLOT[frontLOT];
                if(qptr)
                {
                    /*Enqueuing child nodes if they exist and assigning neighbor to next*/
            ...
            /*Resetting frontLOT and rearLOT variables for using queueLOT again for traversal*/
            frontLOT=ULLONG_MAX; rearLOT=ULLONG_MAX;
        }
    }
}
