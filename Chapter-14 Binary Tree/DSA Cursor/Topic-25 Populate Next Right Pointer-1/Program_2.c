...
void populateNext(struct TreeNode** root_add)
{
    ...
    struct TreeNode* ptr=(*(root_add)), *head=(*(root_add)), *headNext=(*(root_add))->left;
    while(head)
    {
        /*Assigning right child of ptr node to next field of left child and
        if next of ptr exists then assigning left of next to right of current*/
        if(ptr->left) (ptr->left)->next=ptr->right; else break;
        if(ptr->next) (ptr->right)->next=(ptr->next)->left;
        ptr=ptr->next;

        /*Checking whether ptr becomes NULL or not, if it does then incrementing the level*/
        if(!ptr)
        {
            /*Checking whether headNext is NULL or not if yes then breaking the loop*/
            if(!headNext) break; else{ptr=head=headNext; headNext=headNext->left;}
        }
    }
}
