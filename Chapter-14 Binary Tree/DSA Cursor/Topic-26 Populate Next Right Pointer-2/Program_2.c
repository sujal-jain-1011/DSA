...
next pointer and defining a function that will create a new node for the binary tree when called*/
struct TreeNode{
    int val;
...
struct TreeNode* createTreeNode(int data)
{
    ...
    and initializing the left, right child pointers and next pointer to NULL*/
...
/*Function to populate next pointer in each node of the perfect binary tree*/
void populateNext(struct TreeNode** root_add)
{
    /*Handling corner case: when binary tree is empty in that case we are going to return*/
    if(!(*(root_add)))
    {
        /*Displaying user friendly message for emptiness and returning*/
        printf("Binary tree is empty can't populate next right pointer!\n");
        return;
    }

    /*Running a loop to populate next pointer of each node in the binary tree*/
    struct TreeNode* ptr=(*(root_add)), *head=(*(root_add)),
    *headNext=(*(root_add))->left?(*(root_add))->left:(*(root_add))->right, *nptr;
    while(head)
    {
        /*Assigning reference of right child to the next of left child of current node then
        assigning left child of next node to the next field of right child of current node*/
        if(ptr->left)
        {
            /*Traversing the next level of the binary tree to find suitable node to assign*/
            if(ptr->right) ptr->left->next=ptr->right;
            else
            {
                /*If we are in this else block then we need to do the traversal*/
                nptr=ptr->next;
                while(nptr)
                {
                    /*Checking whether left or right child of next node of ptr exists or not*/
                    if(nptr->left){ptr->left->next=nptr->left; break;}
                    else if(nptr->right){ptr->left->next=nptr->right; break;}
                    nptr=nptr->next;
                }
            }
        }
        if(ptr->right)
        {
            /*If we are in this if block that means right child of current node does exist
            and we need to iterate the next level of the binary tree to link next of right*/
            nptr=ptr->next;
            while(nptr)
            {
                /*Checking whether left or right child of next node of ptr exists or not*/
                if(nptr->left){ptr->right->next=nptr->left; break;}
                else if(nptr->right){ptr->right->next=nptr->right; break;}
                nptr=nptr->next;
            }
        }ptr=ptr->next;

        /*Updating head pointer/reference to traverse next level of the binary tree*/
        if(!ptr)
        {
            /*Checking whether headNext node exists or not if not then breaking the loop
            otherwise assigning headNext to head and ptr and updating headNext reference*/
            if(headNext)
            {
                /*Updating ptr, head and headNext references for next iteration*/
                ptr=head=headNext; struct TreeNode* hN=headNext;
                while(hN && (!(hN->left) && !(hN->right))) hN=hN->next;
                if(hN) headNext=hN->left?hN->left:hN->right; else break;
            }
            else break;
        }
    }
...
void levelOrderTraversal(struct TreeNode** root_add)
{
    ...
    else
    {
        ...
        {
            /*Enqueuing root node and NULL into the created circular queue and
            ...
            {
                /*Getting and removing the front element of the queue, printing it's value
                ...
                if(qptr)
                {
                    /*Printing the value of the current node along with next node and enqueuing child nodes if they exist*/
                printf("%d ", qptr->val); if(qptr->next) printf("%d ", (qptr->next)->val); else printf("NULL");
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        /*Calling populateNext function to populate next pointer in each node
        and then calling levelOrderTraversal function to print nodes of BT*/
        populateNext(&root); printf("The level order traversal of binary tree is:\n");
