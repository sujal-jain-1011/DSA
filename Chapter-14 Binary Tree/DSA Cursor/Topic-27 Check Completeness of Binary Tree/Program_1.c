...
and we are also going to check whether the binary tree is complete binary tree or not*/
...
/*Function to check whether the binary tree is complete binary tree or not*/
unsigned short int checkCompleteness(struct TreeNode** root_add)
{
    ...
    else
    {
        ...
        else
        {
            ...
            unsigned short int isLastNode=0;
            enqueue((*root_add), &queLOT, &frontLOT, &rearLOT);
            enqueue(NULL, &queLOT, &frontLOT, &rearLOT);
            while(!isEmpty(frontLOT, rearLOT))
            {
                /*Getting and removing the front element of the queue, checking for completeness
                ...
                if(qptr && !isLastNode)
                {
                    /*Checking for completeness of binary tree and enqueuing children of current node if they exist*/
                    if(!(qptr->left) && !(qptr->right)) isLastNode=1;
                    else if(qptr->left && !(qptr->right)){isLastNode=1; enqueue(qptr->left, &queLOT, &frontLOT, &rearLOT);}
                    else if(!(qptr->left) && qptr->right){free(queLOT); return 0;}
                    else if(qptr->left && qptr->right)
                    {
                        /*If we are in this else block that means both the left and right child exists and we need to enqueue them*/
                        enqueue(qptr->left, &queLOT, &frontLOT, &rearLOT); enqueue(qptr->right, &queLOT, &frontLOT, &rearLOT);
                    }
                }
                else if(qptr && isLastNode)
                {
                    /*Since we have already encountered the last node of binary tree all the upcoming
                    nodes of the binary tree must be leaf nodes so we are going to check for that*/
                    if(qptr->left || qptr->right){free(queLOT); return 0;}
                }
                else
                {
            ...
            /*Deallocating memory to created queue used in this specific function and returning one to caller*/
            free(queLOT); return 1;
        }
    }
}
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        /*Calling checkCompleteness function to check whether binary tree is complete or not*/
        if(checkCompleteness(&root)) printf("Binary tree is complete binary tree!\n");
        else printf("Binary tree is not a complete binary tree!\n");
