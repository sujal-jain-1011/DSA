...
and we are also going to populate next pointer in each node of the binary tree*/
...
time.h header file for getting the time in seconds for generating random sequence*/
#include<limits.h>
#include<stdio.h>
#include<stdlib.h>
#include<time.h>
...
long long unsigned int capacity, front=ULLONG_MAX, rear=ULLONG_MAX, frontLOT=ULLONG_MAX, rearLOT=ULLONG_MAX; static unsigned short counter=1;

/*Defining a structure for binary tree node that contains data, left, right child pointers and
...
struct TreeNode* createTreeNode(int data)
{
    ...
    /*Assigning the data passed to function to the val field of the new node
    and initializing the left, right child pointers and random pointer to NULL*/
...
short unsigned int createBinaryTree()
{
    ...
    randomArray[randomIter++]=root_start;
    ...
    while(iter<capacity)
    {
        ...
        if(!currentTreeNode->left)
        {
            ...
            randomArray[randomIter++]=left_child; enqueue(left_child, &que, &front, &rear);
        }
        ...
        if(iter<capacity && !currentTreeNode->right)
        {
            ...
            randomArray[randomIter++]=right_child; enqueue(right_child, &que, &front, &rear);
        }
    }
...
/*Function to embedd nodes of clone binary tree in b/w nodes of given binary tree*/
void embedd(struct TreeNode** root_add)
{
    /*Declaring base case for recursion: when binary tree is/becomes empty*/
    if((*(root_add)))
    {
        /*Dynamically allocating memory for new node for the clone binary tree node,
        copying the value from corresponding node of the given binary tree, embedding it
        as the left child of original node and calling embedd for left and right subtrees*/
        struct TreeNode* newNode=createTreeNode((*(root_add))->val);
        struct TreeNode* leftChild=(*(root_add))->left, *rightChild=(*(root_add))->right;
        (*(root_add))->left=newNode; newNode->left=leftChild; newNode->right=rightChild;
        embedd(&(newNode->left)); embedd(&(newNode->right));
    }
}

/*Function to separate cloned binary tree from original binary tree*/
void separate(struct TreeNode** root_add, struct TreeNode** newRoot, struct TreeNode* ptr)
{
    /*Declaring base case for recursion: when binary tree is/becomes empty*/
    if((*(root_add)))
    {
        /*Checking whether newRoot is nullptr or not if yes then initializing it
        linking left and right child to original node and to clone node resp.*/
        if(!(*newRoot)) ptr=(*newRoot)=(*(root_add))->left;
        (*(root_add))->left=ptr->left; (*(root_add))->right=ptr->right;
        ptr->left=ptr->left?ptr->left->left:NULL;
        ptr->right=ptr->right?ptr->right->left:NULL;
        
        /*Calling separate function recursively for left and right subtrees of current node*/
        separate(&((*(root_add))->left), newRoot, ptr->left);
        separate(&((*(root_add))->right), newRoot, ptr->right);
    }
}

/*Function to print the level order traversal of binary tree using queue data structure*/
struct TreeNode* clone(struct TreeNode** root_add)
{
    ...
    else
    {
        ...
        {
            /*Calling embedd function to mutate given binary tree*/
            embedd(root_add);
            
            /*Enqueuing root node into the created circular queue and
            then iterating the binary tree to traverse all of it's nodes*/
            enqueue((*root_add), &queLOT, &frontLOT, &rearLOT);
            while(!isEmpty(frontLOT, rearLOT))
            {
                /*Getting and removing front element from queue and populating random pointer*/
                struct TreeNode* ptr=dequeue(&queLOT, &frontLOT, &rearLOT);
                
                /*Populating random pointer in each node and enqueuing only original nodes in the queue*/
                if(ptr->random) ptr->left->random=ptr->random->left;
                if(ptr->left->left) enqueue((ptr->left)->left, &queLOT, &frontLOT, &rearLOT);
                if(ptr->left->right) enqueue((ptr->left)->right, &queLOT, &frontLOT, &rearLOT);
            }

            /*Calling separate function to separate cloned binary tree from original binary tree*/
            struct TreeNode* newRoot=NULL, *iterPtr=NULL; separate(root_add, &newRoot, iterPtr);
            
            /*Deallocating memory to created queue used in this specific function and
            returning the root node of the cloned binary tree to the caller function*/
            free(queLOT); return newRoot;
        }
    }
}
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        /*Calling clone function to clone the given binary tree
        and then preorder function to print the preorder traversal*/
        struct TreeNode* cloneRoot=clone(&root);
        if(cloneRoot)
        {
            /*Displaying user friendly message and printing it's preorder traversal*/
            printf("Binary tree is cloned successfully and it's preorder traversal is:\n");
            preorder(&cloneRoot);
        }
        else printf("Binary tree is not cloned successfully!\n");

        /*Deallocating memory to created binary tree and
        the queue used for creating the binary tree*/
        freeBinaryTree(&root);
        freeBinaryTree(&cloneRoot);
        free(que);
    }
