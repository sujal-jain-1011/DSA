...
random library for generating random indices for array containing pointers*/
#include<iostream>
#include<limits.h>
#include<new>
#include<random>
...
class BinaryTree{
    ...
    public:
        ...
        bool createBinaryTree()
        {
            ...
            randomArray=new struct TreeNode*[capacity];
            if(!randomArray){cout<<"Memory allocation failed for random array!"<<endl; return 0;}
            ...
            mt19937_64 gen(random_device{}()); populateRandomOriginal(&root_start, gen); delete[] randomArray;
        ...
        void populateRandomOriginal(struct TreeNode** root_add, mt19937_64& gen)
        {
            ...
            if((*(root_add)))
            {
                ...
                uniform_int_distribution<llui> dist(0, capacity-1); llui randomIndex=dist(gen);
                (*(root_add))->random=randomArray[randomIndex];
                populateRandomOriginal(&((*(root_add))->left), gen);
                populateRandomOriginal(&((*(root_add))->right), gen);
            }
        }
        ...
        void preorder(struct TreeNode** root_add)
        {
            ...
            {    
                ...
                cout<<(*root_add)->val<<" ";
                if((*(root_add))->random)
                cout<<"TreeNode value pointed to by random pointer of this TreeNode:"<<((*(root_add))->random)->val<<endl;
                else cout<<"TreeNode pointed to by random pointer of this TreeNode is NULL!"<<endl;
