...
and we are also going to serialize and deserialize the constructed binary tree*/
...
/*Function to convert a given integer into a string using the sprintf function of C*/
char* to_string(int num, char* buffer) {sprintf(buffer, "%d", num); return buffer;}

/*Function to serialize the given binary tree by constructing the encoded string*/
void serialize(struct TreeNode** root_add, char* cptr, llui* itr)
{
    /*Declaring base case for recursion: when binary tree is/becomes empty, then we'll return "#"*/
    if((*root_add))
    {
        /*Declaring a string to store the serialization of the current node and then calling the serial-
        ize function recursively for the left and right subtree of the current node in preorder manner*/
        char buffer[6]; to_string((*(root_add))->val, buffer); short unsigned int cIter=0;
        while(buffer[cIter]) {cptr[(*itr)++]=buffer[cIter++];} cptr[(*itr)]=','; (*itr)+=1;
        serialize(&((*(root_add))->left), cptr, itr); serialize(&((*(root_add))->right), cptr, itr);
    }
    else {cptr[(*itr)++]='#'; cptr[(*itr)++]=',';}
}

/*Function to deserialize the binary string: constructing binary tree from serialized string*/
struct TreeNode* deserialize(char* data, llui* itr)
{
    /*Declaring base case for recursion: when we encounter "#" in string, then we'll return nullptr*/
    if(data[(*itr)]!='#')
    {
        /*Creating a node for the current character in the string and then calling the function deserialize
        recursively for the left and right subtree of the current node like for creating those subtrees*/
        int val=0; char sign='\0';
        while(data[(*itr)]!=',')
        {
            /*Checking for sign in the current number of the BT*/
            if(data[(*itr)]=='-') {sign='-'; (*itr)++;}
            else {val=10*val+data[(*itr)]-'0'; (*itr)++;}
        }
        val=sign?(-val):val; (*itr)+=1;

        /*Dynamically allocating memory for new node of BT and calling deserialize recursively for subtrees*/
        struct TreeNode* root=createTreeNode(val); root->left=deserialize(data, itr);
        root->right=deserialize(data, itr); return root;
    }
    else {(*itr)+=2; return NULL;}
}

/*Function to check whether two binary trees are identical or not*/
short unsigned int isIdentical(struct TreeNode** root_add_1, struct TreeNode** root_add_2)
{
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        /*Dynamically allocating memory to store the encoded string for
        the given binary tree and calling serialize function for encoding*/
        char* str=(char*)malloc(sizeof(char)*(4*capacity+1));
        if(!str) printf("Memory allocation from heap failed!\n");
        else
        {
            /*Calling serialize function for serialization of binary tree
            then calling deserialize function for deserializing the binary
            tree and then checking for similarity of deserialized BT*/
            llui iter=0; serialize(&root, str, &iter); iter=0;
            for(llui it=0; it<4*capacity+1; it++) printf("%c", str[it]); printf("\n");
            struct TreeNode* newRoot=deserialize(str, &iter);
            if(isIdentical(&root, &newRoot)) printf("Right\n"); else printf("Wrong\n");

            /*Deallocating memory to binary tree created from deserialization*/
