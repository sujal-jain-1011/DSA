...
and we are also going to find number of turns from soure node to destination node*/

/*Including limits.h header file for using ULLONG_MAX, stdio.h header file for input
and output functions and stdlib.h header file for memory allocation functions and
including string.h for using strcpy function to copy one string onto another*/
#include<limits.h>
#include<stdio.h>
#include<stdlib.h>
#include<string.h>

/*Typedefining long long unsigned int as llui and short unsigned int as sui*/
...
/*Function to find maximum of two long long unsigned integers*/
llui max(llui a, llui b){return a>=b?a:b;}

/*Function to find the height of the binary tree using recursive approach*/
long long unsigned int height(struct TreeNode** root_add)
{
    /*Declaring base case for recursion: when binary tree is/becomes empty, then we are going to return zero*/
    if(!(*(root_add))) return 0; else return max(height(&((*(root_add))->left)), height(&((*(root_add))->right)))+1;
}

/*Function to find the lowest common ancestor of the given nodes in binary tree*/
struct TreeNode* lowAnc(struct TreeNode** root_add, int a, int b)
{
    /*Declaring base case for recursion: when binary tree is/becomes empty*/
    if((*(root_add)))
    {
        /*Checking whether the current node value matches with any of the given
        values or not, if yes then we are going to return current node from here */
        if((*(root_add))->val==a || (*(root_add))->val==b) return (*(root_add));

        /*Calling lowAnc function recursively for left and right subtree of current node for finding the nodes
        and checking whether any of them returned non null or not, if not then returning according to cases*/
        struct TreeNode* lf=lowAnc(&((*(root_add))->left), a, b), *rh=lowAnc(&((*(root_add))->right), a, b);
        if(!(lf) && !(rh)) return NULL; else if(lf && rh) return (*(root_add)); else return lf?lf:rh;
    }
}

/*Function to construct path (left, right instructions) from lca to given node in binary tree*/
void path(struct TreeNode** root_add, int tgt, char** ptr, char** temp, llui* iter, sui* fg)
{
    /*Declaring base case for recursion: when binary tree is/becomes empty or target is found*/
    if((*(root_add) && !(*fg)))
    {
        /*Checking whether the current node value equals the given target value or not,
        if yes then we are not going to return from here without incrementing the size*/
        if((*(root_add))->val==tgt){strcpy(*ptr,*temp); *fg=1; return;}

        /*Calling path function recursively for the left and right subtree of current node by
        inserting the left or right marker (if left and right subtree) exists and then remove*/
        if((*(root_add))->left) {(*temp)[(*iter)++]='L'; path(&((*(root_add))->left), tgt, ptr, temp, iter, fg); (*temp)[(*iter)--]='\0';}
        if((*(root_add))->right) {(*temp)[(*iter)++]='R'; path(&((*(root_add))->right), tgt, ptr, temp, iter, fg); (*temp)[(*iter)--]='\0';}
    }
}

/*Function to find the number of turns in binary tree from source node to destination node*/
long long unsigned int noOfTurns(struct TreeNode** root_add, int first, int second)
{
    /*Since the shortest path between two nodes in the binary tree, passes through
    lowest common ancestor we first need to find the lowest common ancestor of nodes*/
    struct TreeNode* lca=lowAnc(root_add, first, second);

    /*Declaring four character arrays to store the lca to target node path in binary tree
    since we are going to predefine the size of the character arrays we are going to call
    a helper function to get the height of the binary tree which will be the size of array*/
    llui h=height(root_add); llui iter1=0, iter2=0; short unsigned int fg1=0, fg2=0;
    char* tm1=(char*)malloc((h+2)*sizeof(char)); for(llui it=0; it<h+2; it++) tm1[it]=0;
    char* tm2=(char*)malloc((h+2)*sizeof(char)); for(llui it=0; it<h+2; it++) tm2[it]=0;
    char* pm1=(char*)malloc((h+2)*sizeof(char)); for(llui it=0; it<h+2; it++) pm1[it]=0;
    char* pm2=(char*)malloc((h+2)*sizeof(char)); for(llui it=0; it<h+2; it++) pm2[it]=0;

    /*Calling helper function path to get the path from lca node to given target node in binary tree
    and iterating the populated arrays to count the total number of turns in the given binary tree*/
    path(&lca, first, &pm1, &tm1, &iter1, &fg1); path(&lca, second, &pm2, &tm2, &iter2, &fg2);
    llui size1=0, it1=0; while(pm1[it1]){size1+=1; it1+=1;} llui turn1=0;
    llui size2=0, it2=0; while(pm2[it2]){size2+=1; it2+=1;} llui turn2=0;

    /*Iterating the populated permanent vectors to find the individual number of turns*/
    if(size1>0){for(llui it=0; it<size1-1; it++) if(pm1[it]!=pm1[it+1]) turn1+=1;}
    if(size2>0){for(llui it=0; it<size2-1; it++) if(pm2[it]!=pm2[it+1]) turn2+=1;}

    /*Counting the total number of turns in the given binary tree, and returning that to caller*/
    if(!size1) return turn2; else if(!size2) return turn1; else if(pm1[0]==pm2[0])
    return turn1+turn2; else if(pm1[0]!=pm2[0]) return turn1+turn2+1;
}
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        /*Getting the user input for the node values in the binary tree which must be present in binary tree
        otherwise there can be undefined errors and calling noOfTurns function to count total number of turns*/
        int fr, sc; printf("Enter two node values present in binary tree:"); scanf("%d %d", &fr, &sc);
        printf("Number of turns between %d and %d node of binary tree is:%llu\n", fr, sc, noOfTurns(&root, fr, sc));
