...
and we are also going to find the number of univalued subtrees in the given binary tree*/
...
class BinaryTree{
    ...
    public:
        ...
        /*Member function of class: to find the number of univalued subtrees in the given binary tree*/
        int uniValue(struct TreeNode** root_add, llui* count)
        {
            /*Declaring base case for recursion: when binary tree is/becomes empty*/
            if((*(root_add)))
            {
                /*Calling uniValue function recursively for the left and right subtree of the
                current node and checking whether the values returned by left and right subtree
                of current node equals the current node value or not, if not then returning -1*/
                int leftVal=uniValue(&((*(root_add))->left), count), rightVal=uniValue(&((*(root_add))->right), count);
                if(leftVal==-1 || rightVal==-1) return -1;
                else if(((!leftVal) || (*(root_add))->val==leftVal) && ((!rightVal) || (*(root_add))->val==rightVal))
                {
                    /*Incrementing count variable and returning the value of the current node*/
                    (*count)+=1; return (*(root_add))->val;
                }
                else if(!(((!leftVal) || (*(root_add))->val==leftVal) && ((!rightVal) || (*(root_add))->val==rightVal)))
                return -1;

                /*Returning minus one just to avoid warnings*/
                return -1;
            }
            else return 0;
        }
...
int main(void)
{
    ...
    if(BT.createBinaryTree())
    {
        ...
        /*Calling uniValue function to count number of single valued subtrees*/
        llui count=0;  BT.uniValue(&(BT.root), &count);
        cout<<"Number of univalued subtrees in binary tree:"<<count<<endl;
