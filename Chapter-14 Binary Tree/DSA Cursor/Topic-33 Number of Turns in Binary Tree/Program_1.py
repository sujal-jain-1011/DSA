...
class BinaryTree:
        ...
        def uniValue(self, root_add, count):
            ...
            if(root_add):
                ...
                leftVal, rightVal=self.uniValue(root_add.left, count), self.uniValue(root_add.right, count)
                if(leftVal==-1 or rightVal==-1):
                    return -1
                elif(((not(leftVal)) or root_add.val==leftVal) and ((not(rightVal) or root_add.val==rightVal))):
                    #Incrementing count variable and returning the value of the current node
                    count[0]+=1
                    return root_add.val
                elif(not(((not(leftVal)) or root_add.val==leftVal) and ((not(rightVal) or root_add.val==rightVal)))):
                    return -1
            else:
                return 0
...
if(BT.createBinaryTree()):
    ...
    count=[0]
    BT.uniValue(BT.root, count)
    print(f"Number of univalued subtrees in binary tree:{count[0]}")
else:
