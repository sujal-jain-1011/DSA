...
/*Function to find the height of binary tree using recursive approach*/
long long unsigned int height(struct TreeNode** root_add)
{
    ...
    if(!(*root_add)) return 0;
    else return max(height(&((*root_add)->left)), height(&((*root_add)->right)))+1;
}
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        printf("The height of binary tree is:%llu\n", height(&root));
    }
