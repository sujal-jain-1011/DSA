...
/*Function to find the minimum element in binary tree using recursive approach*/
int minInBinaryTree(struct TreeNode** root_add)
{
    ...
    urn INT_MAX else we are going to return minimum of values from left and right subtrees and current node*/
    if(!(*root_add)) return INT_MAX;
    else return min(min(minInBinaryTree(&((*root_add)->left)), minInBinaryTree(&((*root_add)->right))), (*root_add)->val);
}
...
int main(void)
{
    ...
    if(createBinaryTree())
    {
        ...
        printf("The minimum element in binary tree is:%d\n", minInBinaryTree(&root));
    }
