...
"""Importing deque class for using queue data structure and Enum class from enum submodule and
defining a class to store integer constants for type of data which is going to be stored"""
        ...
        def bfs(self, start):
            ...
            for iter in range(self.capacity):
                ...
                if(not(visited.get((start[iter].dt).val))):
                    ...
                    visited[(start[iter].dt).val]=True
                    que.append(start[iter])
                    while(bool(que)):
...
if(gH.createGraph(DataType.INT)):
    ...
    gH.printGraph()
    print("The BFS traversal of given graph is:")
    gH.bfs(gH.GN)
    print()
else:
