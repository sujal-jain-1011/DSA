...
and we are also going to count the number of connected components in the given graph using the custom queue data structure declared separately in a file*/
...
unsigned int countConnected(graphNode** start)
{
    ...
    unsigned int count=0; GenericMap* visited=map_create();
    ...
    for(long long unsigned int iter=0; iter<capacity; iter++)
    {
        ...
        if(!(*((sui*)(ot))))
        {
            ...
            count+=1; (*((sui*)(ot)))=1; graphNode* root=start[iter]; queuePush(que, &root);
            while(!empty(que))
            {
    ...
    /*Returning count variable which contains the number of connected components in the graph*/
    return count;
}
