...
along with that Queue and LinkedList for using queue data structure*/
...
class Graph{
    ...
    /*Member function of class: to count number of connected components in the given undirected graph*/
    long countConnected(GraphNode[] start)
    {
        ...
        HashMap<Object, Boolean>visited=new HashMap<>(); long count=0;
        ...
        for(long iter=0; iter<capacity; iter++)
        {
            ...
            if(!visited.get((start[(int)iter].dt).val))
            {
                /*Marking the current unvisited node as visited and pushing it into queue*/
                count+=1; visited.put((start[(int)iter].dt).val, true); que.offer(start[(int)iter]);
                while(!que.isEmpty())
                {
                    ...
                    while(ptr!=null)
                    {
                        ...
                        if(!visited.get(((ptr.vertex).dt).val))
                        {
                            /*Pushing the unvisited node into queue and marking the current neighbor node visited*/
                            que.offer((ptr.vertex)); visited.put(((ptr.vertex).dt).val, true);
                        }
                        ptr=ptr.next; 
                    }
                }
...
public class Program_1
{
    ...
    public static void main(String[] args)
    {
        ...
        if(gH.createGraph(dataType.intData))
        {
            ...
            System.out.println("Total number of connected components in graph:"+gH.countConnected(gH.GN));
        }
