...
and we are also going to count the number of connected components in the given graph using the recursive approach (it can be implemented using stack as well)*/
...
void dfsHelper(graphNode* gNode, GenericMap* vt)
{
    ...
    while(ngbd)
    {
        ...
        if(!(*((sui*)(output)))) dfsHelper(ngbd->vertex, vt); ngbd=ngbd->next;
    }
}

/*Function to count number of connected components in the given undirected graph*/
unsigned int countConnected(graphNode** start)
{
    ...
    GenericMap* visited=map_create(); unsigned int count=0;
    ...
    for(long long unsigned int iter=0; iter<capacity; iter++)
    {
        ...
        if(!(*((sui*)(outPut)))){count+=1; dfsHelper(start[iter], visited);}
    }

    /*Returning count variable which contains the count of connected components in the graph*/
    return count;
}
...
int main(void)
{
    ...
    if(createGraph(0))
    {
        ...
        printf("Total number of connected components in graph:%u\n", countConnected(GN));
    }
