...
unordered map file for using user defined custom unordered map*/
#include<iostream>
#include<new>
#include<limits.h>
...
class Graph{
    ...
    public:
        ...
        void dfsHelper(graphNode* gNode, unordered_map<void*, bool>& vt)
        {
            ...
            while(ngbd)
            {
                ...
                if(!vt[((ngbd->vertex)->dt)->val]) dfsHelper(ngbd->vertex, vt); ngbd=ngbd->next;
            }
        }
        ...
        unsigned int countConnected(graphNode** start)
        {
            ...
            unordered_map<void*, bool> visited; unsigned int count=0;
            ...
            for(long long unsigned int iter=0; iter<capacity; iter++)
            {
                ...
                if(!visited[(start[iter]->dt)->val]){count+=1; dfsHelper(start[iter], visited);}
            }
...
int main(void)
{
    ...
    if(gH.createGraph(dataType::intData))
    {
        ...
        cout<<"Total number of connected components in graph:"<<gH.countConnected(gH.GN)<<endl;
    }
