...
and we are also going to print the vector of connected components in the given graph using the custom queue data structure declared separately in a file*/
...
file for using user defined custom unordered map and queue.h for using queue
and vector.h header file for using C++ style vector (dynamic array)*/
#include<stdio.h>
#include<stdlib.h>
#include<limits.h>
...
/*Function to print connected components in the given undirected graph*/
void printConnected(graphNode** start)
{
    ...
    GenericMap* visited=map_create();
    for(llui it=0; it<capacity; it++)
    {
        /*Inserting the node along with it's status i.e. visited or not into map*/
        sui st=0; map_put(&visited, ((start[it]->dt)->val), ((start[it]->dt)->sizeOfVal), (void*)(&st), sizeof(st));
    }
    ...
    llui count=0; GenericQueue* que=(GenericQueue*)malloc(sizeof(GenericQueue)); queueInit(que, sizeof(graphNode*));
    for(long long unsigned int iter=0; iter<capacity; iter++)
    {
        ...
        if(!(*((sui*)(ot))))
        {
            ...
            and pushing this element into the vector to print connected components*/
            (*((sui*)(ot)))=1; graphNode* root=start[iter]; queuePush(que, &root);
            vector* vec=(vector*)malloc(sizeof(vector));
            vecInit(vec, sizeof(graphNode*)); graphNode* curr=start[iter]; vecPush(vec, &curr);
            while(!empty(que))
            {
                ...
                graphNode* curr=*(graphNode**)queueFront(que); queuePop(que);
                AdjNeigh* ptr=curr->adj;
                while(ptr)
                {
                    ...
                    void* outPut;
                    if(map_get(visited, ((ptr->vertex->dt)->val), ((ptr->vertex->dt)->sizeOfVal), &outPut))
                    {
                        /*Entering inside the conditional only after checking the outside conditional*/
                        if(!(*((sui*)outPut)))
                        {
                            /*Pushing the unvisited node into queue and marking the current neighbor node visited*/
                            graphNode* ngbd=ptr->vertex; vecPush(vec, &(ngbd));
                            queuePush(que, &(ptr->vertex)); (*((sui*)outPut))=1;
                        }
                    }
                    ptr=ptr->next; 
                }
            ...
            /*Printing connected components of the graph stored in the temporary vector*/
            size_t sz=vec->length; printf("Nodes of connected component %llu are:\n", ++count);
            for(size_t it=0; it<sz; it++) {graphNode* node=*(graphNode**)vecGet(vec, it); printDT(node->dt); printf(" ");}
            printf("\n");
        }
    }
}
...
int main(void)
{
    ...
    if(createGraph(0))
    {
        ...
        printf("Connected components in the graph are:\n");
        printConnected(GN);
    }
