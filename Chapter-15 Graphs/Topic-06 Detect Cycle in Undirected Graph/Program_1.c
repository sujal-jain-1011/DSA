...
and we are also going to detect cycle in the given graph using the recursive approach (it can be implemented using stack as well) using unordered set*/
...
file for using user defined custom unordered map and user defined unordered set*/
#include<stdio.h>
#include<stdlib.h>
#include<limits.h>
...
void dfs(graphNode* gNode, GenericMap* vt, GenericSet* anc, graphNode* pNode, sui* cyc)
{
    /*Declaring base case for recursion: when cycle has been detected in the graph*/
    if(!(*(cyc)))
    {
        ...
        void* ot; map_get(vt, ((gNode->dt)->val),  ((gNode->dt)->sizeOfVal), &ot); *((sui*)(ot))=1;
        AdjNeigh* ngbd=gNode->adj; set_insert(&anc, &gNode, sizeof(graphNode*));
        while(ngbd && !(*(cyc)))
        {
            /*Checking whether the neighbor of the current node already exists in unordered set or not*/
            if(ngbd->vertex!=pNode && set_contains(anc, &(ngbd->vertex), sizeof(ngbd->vertex))) (*(cyc))=1;
            else
            {
                ...
                void* output; map_get(vt, (((ngbd->vertex)->dt)->val), (((ngbd->vertex)->dt)->sizeOfVal), &output);
                if(!(*((sui*)(output)))) dfs(ngbd->vertex, vt, anc, gNode, cyc);
            }
            ngbd=ngbd->next;
        }
    }
...
/*Function to perform the depth first search traversal of the given graph*/
short unsigned int detectCycle(graphNode** start)
{
    ...
    GenericMap* visited=map_create(); GenericSet* ancestor=set_create(); sui cycle=0;
    ...
    for(long long unsigned int iter=0; iter<capacity; iter++)
    {
        ...
        if(!(*((sui*)(outPut))))
        {
            /*Checking for cycle existence in the graph and after that clearing the set*/
            if(!cycle) dfs(start[iter], visited, ancestor, NULL, &cycle); else break;
            set_clear(&ancestor);
        }
    }

    /*Returning cycle variable which contains result of whether the cycle is present in graph or not*/
    return cycle;
}
...
int main(void)
{
    ...
    if(createGraph(0))
    {
        ...
        if(detectCycle(GN)) printf("Cycle is present in the graph\n");
        else printf("Cycle is not present in the graph\n");
    }
