...
class Graph{
    ...
    public:
        ...
        void dfsHelper(graphNode* gNode, unordered_map<void*, bool>& vt)
        {
            ...
            printDT(gNode->dt); cout<<" "; AdjNeigh* ngbd=gNode->adj;
            while(ngbd)
            {
        ...
        void dfs(graphNode** start)
        {
            ...
            for(long long unsigned int iter=0; iter<capacity; iter++)
            {
                ...
                if(!visited[(start[iter]->dt)->val]) dfsHelper(start[iter], visited);
            }
        }
};
...
int main(void)
{
    ...
    if(gH.createGraph(dataType::intData))
    {
        ...
        cout<<"The DFS traversal of given graph is:"<<endl; gH.dfs(gH.GN); cout<<endl;
    }
