...
and we are also going to check whether the given graph is bipartite graph or not using the recursive approach (it can be implemented using stack as well)*/
...
header file for using predefined max and min limits of data types, generic_map.h
file for using user defined custom unordered map defined in a separate file*/
#include<stdio.h>
#include<stdlib.h>
#include<limits.h>
#include "generic_map.h"

/*Typedefining long long unsigned int as llui and short unsigned int as sui, short signed int as
ssi and defining an enum to store integer constants for type of data which is going to be stored*/
typedef long long unsigned int llui; typedef short unsigned int sui;
typedef short signed int ssi; enum dataType{intData, doubleData, charData};
...
/*Function to print the depth first search traversal of the given graph
this function is a helper function which will be called from dfs function*/
void dfsHelper(graphNode* gNode, GenericMap* vt, GenericMap* cl, ssi pColor, sui* bp)
{
    /*Checking whether the color of the current node matches with the parent or not,
    if color of current matches with that of parent then we are going to return false
    and we are also going to check whether the current node is visited or not*/
    void* otClr=NULL; map_get(cl, ((gNode->dt)->val), ((gNode->dt)->sizeOfVal), &otClr);
    if(otClr && (*((ssi*)(otClr)))==pColor){(*bp)=0; return;}
    ...
    void* ot; map_get(vt, ((gNode->dt)->val), ((gNode->dt)->sizeOfVal), &ot);
    if((*((sui*)(ot)))) return; *((sui*)(ot))=1; AdjNeigh* ngbd=gNode->adj;
    ssi pClr=!(otClr)?(pColor!=-1?(!pColor):0):(*((ssi*)(otClr))); ssi cClr=pColor!=-1?(!pColor):0;
    if(!otClr) map_put(&cl, ((gNode->dt)->val), ((gNode->dt)->sizeOfVal), (void*)&cClr, sizeof(ssi));
    while(ngbd)
    {
        /*We are going to call dfsHelper function to process this neighbor node of current node
        and then we are going to check whether the bp is zero or not, if yes then return from here*/
        dfsHelper(ngbd->vertex, vt, cl, pClr, bp); if((*bp)==0) return; ngbd=ngbd->next;
    }
}

/*Function to check whether the given graph is a bipartite graph or not*/
short unsigned int bipartite(graphNode** start)
{
    ...
    GenericMap* visited=map_create(), *color=map_create(); sui bp=1;
    for(long long unsigned int it=0; it<capacity; it++)
    {
        /*Inserting the node along with it's status i.e. visited or not into map*/
        sui st=0; map_put(&visited, ((start[it]->dt)->val), ((start[it]->dt)->sizeOfVal), (void*)&st, sizeof(sui));
    }
    ...
    for(long long unsigned int iter=0; iter<capacity; iter++)
    {
        /*Checking whether the current node of the graph is visited or not, if not then assigning
        color to it and calling dfsHelper function to check whether the graph is bipartite or not*/
        void* outPut; map_get(visited, (((start[iter])->dt)->val), (((start[iter])->dt)->sizeOfVal), &outPut);
        if(!(*((sui*)(outPut))))
        {
            /*Assigning color to parent element/start element of a connected component of graph and calling
            dfsHelper function to check whether the given graph is a bipartite graph or not using dfs*/
            short signed int clr=0;
            map_put(&color, ((start[iter]->dt)->val), ((start[iter]->dt)->sizeOfVal), (void*)&clr, sizeof(ssi));
            dfsHelper(start[iter], visited, color, -1, &bp); if(!bp) return 0;
        }
    }

    /*Returning one to the caller since we are out of the loop and graph is bipartite*/
    return 1;
}
...
int main(void)
{
    ...
    if(createGraph(0))
    {
        ...
        if(bipartite(GN)) printf("Given graph is bipartite graph\n");
        else printf("Given graph is not a bipartite graph\n");
    }
